...
export const SELF_MAINTAINER_PROPOSAL_WORKER_VERSION = 'self-maintainer-proposal-worker-1.0.1';
...
export async function runSelfMaintainerProposalWorker({
  ...
  for (const candidate of candidates) {
    ...
    if (taskReasons.length) {
      return fail(['claimed-task-not-self-maintainer', ...taskReasons], 'TASK_REJECTED', { issueNumber: candidate.issueNumber, taskId: task?.taskId || null });
    }
    ...
    if (!heartbeat?.ok) {
      return fail(['proposal-relay-heartbeat-failed', ...(heartbeat?.reasonCodes || [])], 'LEASE_LOST', { issueNumber: candidate.issueNumber, taskId: task.taskId });
    }
    ...
    const submitted = await submitGithubRelayResult({
      ...
      tests: provider.result.testsActuallyRun, artifacts: provider.result.changedArtifacts,
