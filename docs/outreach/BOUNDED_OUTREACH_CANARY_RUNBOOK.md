...
Deploy Web and Worker with PostgreSQL, apply all migrations including `012_outreach_recipient_cooldowns.sql` and `013_outreach_operator_surface.sql`, and keep:
...
For a multi-step workbench sequence, advance the due plan to `ready_for_review`, preview the exact rendered variant, then use `/api/outreach/sequence/approve-step` with fresh route evidence. The endpoint stores a step-bound approval but still makes zero provider calls. A worker sends only an `approved` current step and advances state after a durable provider result.
...
There is no unapproved follow-up. A follow-up requires the stored original `threadId` and RFC message ID, no reply or suppression, and another exact approval. The bounded sequence surface supports up to twelve total steps (initial plus eleven follow-ups); do not raise volume from one canary until delivery, unsubscribe, suppression, bounce, reply, and duplicate-reconciliation behavior has been inspected.
