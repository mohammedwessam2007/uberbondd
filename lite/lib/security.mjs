// Vendored from src/security.mjs so the Vercel deployment (root directory: lite/)
// is fully self-contained. Adds an injectable `lookup` for offline tests.
// The GitHub Actions worker additionally re-validates every request through the
// production crawler's own src/security.mjs guard — defense in depth.
...
// Matches any explicit "<scheme>://" prefix (ftp://, javascript://, etc.), not just http(s).
// A bare `startsWith('http')` check is insufficient: for input like 'ftp://example.com' it
// is false, so the string gets `https://` prepended, producing 'https://ftp://example.com',
// which the URL parser happily accepts with hostname 'ftp' and protocol 'https:' — silently
// bypassing the scheme rejection below instead of triggering it.
const HAS_SCHEME = /^[a-z][a-z0-9+.-]*:\/\//i;

export async function assertPublicUrl(input, { allowLocal = false, lookup = null } = {}) {
  ...
  const resolve = lookup || ((name, options) => dns.lookup(name, options));
