...
export const C13_INVARIANT_EVALUATOR_V2='uberbond.c13-invariant-cognition-evaluator.v2';
...
export function evaluateInvariantCognitionTransferV2({contractDigest,expectedContractDigest,families=[]}={}){
 const reasons=[];
 ...
 for(const f of Array.isArray(families)?families:[]){
  ...
  const local=[];
  if(current.length!==21||candidate.length!==21||current.some(x=>!Number.isFinite(x)||x<=0)||candidate.some(x=>!Number.isFinite(x)||x<=0))local.push('twenty-one-matched-positive-latency-rounds-required');
  if(!f.currentSemanticDigest||f.currentSemanticDigest!==f.candidateSemanticDigest)local.push('semantic-identity-required');
  if(!f.currentAuthorityDigest||f.currentAuthorityDigest!==f.candidateAuthorityDigest)local.push('authority-identity-required');
  if(!f.currentTruthBoundaryDigest||f.currentTruthBoundaryDigest!==f.candidateTruthBoundaryDigest)local.push('truth-boundary-identity-required');
  ...
  if(!local.length){
    ...
    const R=rng(hash({contractDigest,familyId:f.familyId,version:C13_INVARIANT_EVALUATOR_V2})),boots=[];
    for(let b=0;b<10000;b++){const ca=[],cb=[];for(let i=0;i<current.length;i++){const j=Math.floor(R()*current.length);ca.push(current[j]);cb.push(candidate[j]);}boots.push(score(median(ca)/median(cb)));}
 ...
 return {ok:reasons.length===0,status:supported?'C13_INVARIANT_TRANSFER_V2_SUPPORTED':'C13_INVARIANT_TRANSFER_V2_NOT_SUPPORTED',supported,robustImprovedFamilies:improved,requiredRobustImprovedFamilies:2,families:rows,reasonCodes:[...new Set(reasons)],promotionAuthority:'NONE',businessEffectAuthority:'NONE',externalEffectAuthority:'NONE',asiStatus:'SYSTEM_LEVEL_ASI_NOT_ESTABLISHED',truthBoundary:'C13_V2_TRANSFER_EVALUATION_ONLY__NO_AUTOMATIC_PROMOTION__NO_C21_OR_ASI_CLAIM'};
}
