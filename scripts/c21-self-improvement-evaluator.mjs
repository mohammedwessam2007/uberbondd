...
export function evaluateWholeSystemSelfImprovement({
  ...
  for (const family of Array.isArray(families) ? families : []) {
    ...
    const local = [];
    if (current.length !== 21 || candidate.length !== 21 || current.some(value => !Number.isFinite(value) || value <= 0) || candidate.some(value => !Number.isFinite(value) || value <= 0)) local.push('twenty-one-matched-positive-latency-rounds-required');
    ...
    if (family.wallTimeBudgetMs !== 5000 || family.monetaryCostMicros !== 0 || family.humanAssistanceMinutes !== 0 || family.externalTools !== 'NONE') local.push('frozen-matched-budget-required');
    ...
    if (!local.length) {
      ...
      const random = rng(hash({ contractDigest, taskPopulationHash, familyId: family.familyId, version: C21_SELF_IMPROVEMENT_EVALUATOR_VERSION }));
      const bootstrap = [];
    ...
    rows.push({ familyId: family.familyId, currentScore: 0.5, candidateScore, speedup, scoreInterval: { low: candidateLow, high: candidateHigh }, robustGainVsCurrent: robustGain, passes });
  }
