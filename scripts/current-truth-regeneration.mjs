...
export function executeCurrentTruthRegeneration(){
 ...
 const enforcementRun=run('node',['scripts/enforcement-manifest-admission.mjs']);if(enforcementRun.exitCode!==0)return{ok:false,status:'CURRENT_TRUTH_REGENERATION_REFUSED',reasonCodes:['enforcement-manifest-admission-failed'],generatorResults:{readiness:readinessRun,enforcement:enforcementRun},businessEffectAuthority:'NONE'};
 const coverageRun=run('node',['scripts/sovereign-coverage-matrix.mjs']);if(coverageRun.exitCode!==0)return{ok:false,status:'CURRENT_TRUTH_REGENERATION_REFUSED',reasonCodes:['coverage-generator-failed'],generatorResults:{readiness:readinessRun,enforcement:enforcementRun,coverage:coverageRun},businessEffectAuthority:'NONE'};
 const leafGraphRun=run('node',['scripts/canonical-execution-leaf-materializer.mjs']);if(leafGraphRun.exitCode!==0)return{ok:false,status:'CURRENT_TRUTH_REGENERATION_REFUSED',reasonCodes:['canonical-leaf-graph-generator-failed'],generatorResults:{readiness:readinessRun,enforcement:enforcementRun,coverage:coverageRun,leafGraph:leafGraphRun},businessEffectAuthority:'NONE'};
 ...
 return verifyCurrentTruthRegenerationWithReferenceIntegrity({headSha,readiness,coverage,leafGraph,freeze,trackedPaths:tracked,dirtyPaths:newlyDirtyPaths(before,after||[]),preRegenerationDirtyPaths:before,generatorResults:{readiness:readinessRun,enforcement:enforcementRun,coverage:coverageRun,leafGraph:leafGraphRun}});
}
