...
let httpStatus=0;let view=null;
try{
...
const receipt=compileFounderOpsRuntimeProbeReceipt({expectedSourceCommit:sourceCommit,httpStatus,view,verifierRef,evidenceRef,observedAt:new Date()});
...
console.log(JSON.stringify({ok:receipt.ok,status:receipt.status,sourceCommit:receipt.sourceCommit||sourceCommit,runtimeIdentity:receipt.runtimeIdentity||null,receiptDigest:receipt.receiptDigest||null,output,businessEffectAuthority:'NONE'}));
