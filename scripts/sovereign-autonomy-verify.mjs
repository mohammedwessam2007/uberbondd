...
export const SOVEREIGN_AUTONOMY_VERIFY_VERSION = 'uberbond.sovereign-autonomy-verify.v3';
...
export async function runSovereignAutonomyVerification({ env = process.env, repoRoot = process.cwd(), runProcess = run } = {}) {
  ...
  const controlDir = path.resolve(env.UBERBOND_CONTROL_DIR || '/var/lib/uberbond-control'); const autonomyDir = path.join(controlDir, 'autonomy'); const taskPath = path.join(autonomyDir, 'next-task.json');
  ...
  const task = await readJson(taskPath); const result = await readJson(resultPath); const candidate = result?.codeChangeSet || result?.changeSet || result;
  ...
  await atomicJson(verifiedPath, out); await atomicJson(governancePath, out, 0o640); return { ...out, governanceHandoff: governancePath };
}
