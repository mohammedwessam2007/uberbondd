...
export const SOVEREIGN_CONTEXT_JOURNAL_COMPACTION_VERSION='sovereign-context-journal-compaction-1.0.0';
...
export function compactContextJournalLayeredStore({journalPath,layeredStorePath=null,segmentSize=2048,activatedAt=new Date(),force=false}={}){
  ...
  try{
    ...
    return{ok:true,compactionVersion:SOVEREIGN_CONTEXT_JOURNAL_COMPACTION_VERSION,status:'CONTEXT_JOURNAL_COMPACTION_VERIFIED',previousDescriptorId:current.descriptorId,descriptorId:next.descriptorId,generationId:next.generationId,archiveManifestId:next.archiveManifestId,entryCount:rebound.entryCount,tipDigest:rebound.tipDigest,tailEntryCount:0,priorGenerationRetained:true,businessEffectAuthority:'NONE',externalEffectAuthority:'NONE',externalEffectLedger:zeroEffects(),truthBoundary:'Compaction builds and verifies a new immutable generation, atomically flips only the descriptor pointer, and retains the prior archive, tail, legacy journal, and descriptor identity. No historical bytes are deleted.'};
