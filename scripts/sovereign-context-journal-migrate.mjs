...
export const SOVEREIGN_CONTEXT_JOURNAL_MIGRATION_VERSION='sovereign-context-journal-migration-1.0.0';
...
export function migrateContextJournalToLayeredStore({journalPath,layeredStorePath=null,segmentSize=2048}={}){
  ...
  fs.mkdirSync(path.dirname(root),{recursive:true,mode:0o700});const lock=`${legacyPath}.migration.lock`;let lockFd;
  const stage=`${root}.stage.${process.pid}`;
  try{
    try{lockFd=fs.openSync(lock,'wx',0o600);}catch(error){if(error?.code==='EEXIST')return fail(['journal-migration-lock-held'],'CONTEXT_JOURNAL_MIGRATION_BUSY');throw error;}
    ...
    const snapshot=exportCognitiveJournalSegments({journalPath:legacyPath,outputRoot:path.join(stage,'archive'),segmentSize});if(!snapshot.ok)return fail(['archive-snapshot-export-failed',...(snapshot.reasonCodes||[])]);
    ...
    return{...active,migrationVersion:SOVEREIGN_CONTEXT_JOURNAL_MIGRATION_VERSION,status:'CONTEXT_JOURNAL_LAYERED_MIGRATION_VERIFIED',legacyJournalPath:legacyPath,legacyJournalPreserved:true,truthBoundary:'Migration atomically activates a verified immutable archive plus empty global tail and leaves the original monolithic journal untouched as rollback evidence. It does not delete historical bytes.'};
