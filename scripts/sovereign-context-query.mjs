...
function sourceRoot(candidate){const fallback=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..');const supplied=String(candidate||'').trim();return supplied?path.resolve(supplied):fallback;}
...
export function querySovereignContext({mission,controlDir='/var/lib/uberbond-control',rootDir=null,maxHistory=8,audience='research-worker',generatedAt=new Date()}={}){
  ...
  const historyLimit=Number(maxHistory);if(!Number.isSafeInteger(historyLimit)||historyLimit<0||historyLimit>12)return fail(['valid-context-query-history-limit-required']);
  const root=sourceRoot(rootDir);const control=path.resolve(controlDir);
  const mounted=mountSovereignContext({rootDir:root,journalPath:path.join(control,'context','events.jsonl'),capsuleCachePath:path.join(control,'context','brainstate.json'),mountCachePath:null,mission:query,maxHistoricalEvents:historyLimit,generatedAt});
