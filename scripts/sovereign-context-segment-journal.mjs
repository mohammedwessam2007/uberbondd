...
export const SOVEREIGN_CONTEXT_SEGMENT_JOURNAL_VERSION='sovereign-context-segment-journal-1.0.1';
...
export function exportCognitiveJournalSegments({journalPath,outputRoot,segmentSize=2048}={}){
  ...
  if(fs.existsSync(final)){const existing=verifyCognitiveJournalSegmentSnapshot(final);if(!existing.ok)return fail(['existing-segment-snapshot-invalid',...(existing.reasonCodes||[])],'CONTEXT_JOURNAL_SEGMENT_EXPORT_REFUSED');return{...existing,status:'CONTEXT_JOURNAL_SEGMENT_SNAPSHOT_ALREADY_PRESENT'};}
  fs.mkdirSync(stage,{mode:0o700});
  try{for(const segment of compiled.segments)atomicJson(path.join(stage,segmentFilename(segment)),segment);atomicJson(path.join(stage,'manifest.json'),compiled.manifest);syncDir(stage);fs.renameSync(stage,final);syncDir(root);}catch(error){fs.rmSync(stage,{recursive:true,force:true});return fail(['segment-snapshot-write-failed'],'CONTEXT_JOURNAL_SEGMENT_EXPORT_FAILED',{detail:String(error?.message||error).slice(0,300)});}
  ...
  return{...verified,status:'CONTEXT_JOURNAL_SEGMENT_SNAPSHOT_WRITTEN',truthBoundary:'This is an immutable content-addressed snapshot of the verified journal. It does not replace or truncate the active journal.'};
}
