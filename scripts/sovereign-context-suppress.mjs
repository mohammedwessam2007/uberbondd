...
export function suppressContextEvent({journalPath,layeredStorePath=null,authorizationRoot,targetEventId,authorizationId,observedAt=new Date()}={}){
  ...
  const appended=appendContextJournalRuntime({journalPath,layeredStorePath,compiledEvent:compiled.compiledEvent});if(!appended.ok)return fail(['suppression-journal-append-failed',...(appended.reasonCodes||[])],appended.status);
  return{ok:true,suppressVersion:SOVEREIGN_CONTEXT_SUPPRESS_VERSION,status:'CONTEXT_EVENT_SUPPRESSED_FROM_ORDINARY_RETRIEVAL',targetEventId:target,authorizationId:authId,directiveEventId:appended.eventId,journalSequence:appended.sequence,journalEntryDigest:appended.entryDigest,journalStorageMode:appended.storageMode||journal.storageMode||'LEGACY_JSONL',businessEffectAuthority:'NONE',externalEffectAuthority:'NONE',externalEffectLedger:zeroEffects(),truthBoundary:'This appends a founder-authorized retrieval suppression directive. The target remains in the immutable journal and may remain in backups, replicas, or prior exports.'};
}

if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  const journalPath=process.env.UBERBOND_CONTEXT_JOURNAL_PATH||null;const layeredStorePath=process.env.UBERBOND_CONTEXT_LAYERED_STORE_PATH||null;const authorizationRoot=process.env.UBERBOND_FOUNDER_AUTHORIZATION_ROOT||null;const result=suppressContextEvent({journalPath,layeredStorePath,authorizationRoot,targetEventId:process.argv[2],authorizationId:process.argv[3]});process.stdout.write(`${JSON.stringify(result,null,2)}\n`);if(!result.ok)process.exitCode=2;
}
