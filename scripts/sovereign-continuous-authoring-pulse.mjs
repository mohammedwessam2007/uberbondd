...
export async function runSovereignContinuousAuthoringPulse({
  ...
  if (primary.status !== 'FINITE_ENGINEERING_ALREADY_CLOSED' || primary.taskRequired !== false) return primary;
  ...
  const directive = compileSandwichAutocatalyticDirective({ baseRevision, finiteDirective: primary });
  ...
  if (!task?.taskId || task.consequenceClass !== 'LOCAL_PREPARATION') return fail(['bounded-local-descendant-task-required']);
