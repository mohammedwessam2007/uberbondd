...
import { runSovereignInitiativeRuntime } from './sovereign-initiative-runtime.mjs';

export const SOVEREIGN_INITIATIVE_HOST_VERSION = 'uberbond.sovereign-initiative-host.v1';
const MAX_BYTES = 8_000_000;
const execFileAsync = promisify(execFile);
const zeroEffects = () => structuredClone(ZERO_EXTERNAL_EFFECTS);
function fail(reasonCodes, status='SOVEREIGN_INITIATIVE_HOST_REFUSED', extra={}) {
  return { ok:false, hostVersion:SOVEREIGN_INITIATIVE_HOST_VERSION, status, reasonCodes:[...new Set((reasonCodes||[]).filter(Boolean))], businessEffectAuthority:'NONE', externalEffectAuthority:'NONE', externalEffectLedger:zeroEffects(), ...extra };
}
...
async function maybePumpMesh({ env, root, storeDir, occurrenceKey }) {
  if (String(env.AGENT_MESH_ENABLED||'').toLowerCase() !== 'true') {
    return { ok:true, status:'AGENT_MESH_PUMP_DISABLED', exitCode:null, truthBoundary:'The mission is durably queued, but provider/relay execution remains disabled until the existing Agent Mesh authority/configuration enables it.' };
  }
  const childEnv = {
    ...process.env,
    ...env,
    ...
    AGENT_MESH_OCCURRENCE_KEY:`${occurrenceKey}/pulse/${Date.now()}`,
    AGENT_MESH_MISSION:''
  };
...
export async function runSovereignInitiativeHost({ env=process.env, now=new Date() }={}) {
  ...
  try {
    ...
    if (priorCycle?.ok===true && priorCycle?.status==='NORTH_STAR_INITIATIVE_CYCLE_READY' && priorDispatch?.ok===true) {
      ...
      if (observed.status==='NORTH_STAR_INITIATIVE_RUN_PENDING') {
        const pump=await maybePumpMesh({env,root,storeDir,occurrenceKey:priorDispatch.occurrenceKey});
      ...
      const pump=await maybePumpMesh({env,root,storeDir,occurrenceKey:repaired.occurrenceKey});
    ...
    const runtime=await runSovereignInitiativeRuntime({env:{...env,UBERBOND_INITIATIVE_MISSION_RECEIPT_PATH:missionResultPath},now});
    ...
    const pump=await maybePumpMesh({env,root,storeDir,occurrenceKey:dispatch.occurrenceKey});
    ...
    if (typeof store.close==='function') await store.close().catch(()=>{});
  }
}
