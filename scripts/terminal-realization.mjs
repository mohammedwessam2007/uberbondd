...
export function executeTerminalRealization(){
  ...
  if(failed.length){
    ...
    if(semanticRun?.exitCode!==0&&semanticTribunal){
      const handoff=compileTerminalSemanticRefusalHandoff({truthReceipt,semanticTribunal,runs,sinkAuthorityReport,cutSetReport});
...
if(direct){const result=executeTerminalRealization();const truthRegenerationDiagnostics=result.reasonCodes?.includes('current-truth-regeneration-failed')?result.detail?.truthReceipt||null:null;process.stdout.write(`${JSON.stringify({ok:result.ok,status:result.status,finiteEngineeringClosure:result.separatedStatus?.FINITE_ENGINEERING_CLOSURE||result.finiteEngineeringClosure||'INCOMPLETE',reasonCodes:result.reasonCodes||[],finiteOpenRequirements:Array.isArray(result.finiteOpenRequirements)?result.finiteOpenRequirements.length:0,semanticDiagnostics:result.semanticDiagnostics||null,truthRegenerationDiagnostics,runtimeProofRequiredCuts:result.componentReceipts?.cutSets?.runtimeProofRequiredCuts||[],externalProviderCuts:result.componentReceipts?.cutSets?.externalProviderCuts||[],ownerCustodyCuts:result.componentReceipts?.cutSets?.ownerCustodyCuts||[],output:'artifacts/sovereign/terminal-realization.json'},null,2)}\n`);if(!result.ok)process.exitCode=2;}
