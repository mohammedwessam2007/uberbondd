...
export function executeTerminalRealization(){
  ...
  if(!truthReceipt.ok)return refuse(['current-truth-regeneration-failed'],{truthReceipt});
  ...
  const failed=runs.filter(r=>r.exitCode!==0);if(failed.length)return refuse(['terminal-source-tribunal-generator-failed'],{runs});
  const semanticTribunal=readJson('artifacts/sovereign/semantic-requirement-tribunal.json');
  const sinkAuthorityReport=readJson('artifacts/sovereign/composed-effect-authority-audit.json');
  const cutSetReport=readJson('artifacts/sovereign/sovereign-cut-set-audit.json');
  ...
  const tribunal=compileFiniteClosureTribunal({
    truthReceipt,semanticTribunal,executionGraph,continuationProof,sinkAuthorityReport,recursiveGovernanceReport,cutSetReport,
