...
import { loadRestartRecoveryRuntimeEvidence, loadSourceRepositoryHostRuntimeEvidence, loadWebRuntimeHostRuntimeEvidence } from '../src/runtime-proof-ingestion.mjs';
...
function run(script){const result=spawnSync(process.execPath,[script],{cwd:root,env:process.env,encoding:'utf8',stdio:['ignore','pipe','pipe']});if(result.stdout)process.stdout.write(result.stdout);if(result.stderr)process.stderr.write(result.stderr);return{script,exitCode:result.status??1,error:result.error?String(result.error.message||result.error):null};}
...
function remainingEvidence(expectedSourceCommit){
  const specs=[
    ['MESSAGING_PROVIDER','UBERBOND_MESSAGING_PROVIDER_RECEIPT_PATH'],
    ['PAYMENT_PROVIDER','UBERBOND_PAYMENT_PROVIDER_RECEIPT_PATH'],
    ['DEPLOYMENT_PROVIDER','UBERBOND_DEPLOYMENT_PROVIDER_RECEIPT_PATH'],
    ['CREDENTIAL_CUSTODY','UBERBOND_CREDENTIAL_CUSTODY_RECEIPT_PATH'],
    ['SOVEREIGN_IDENTITY','UBERBOND_SOVEREIGN_IDENTITY_RECEIPT_PATH']
  ];
  return Object.fromEntries(specs.map(([cutId,envName])=>[cutId,loadRemainingCutEvidence({path:process.env[envName]||'',expectedSourceCommit,expectedCutId:cutId})]));
}
...
export function executeTerminalRealization(){
  ...
  const sourceRepositoryEvidence=loadSourceRepositoryHostRuntimeEvidence({path:process.env.UBERBOND_SOURCE_REPOSITORY_HOST_RECEIPT_PATH||'',expectedSourceCommit:truthReceipt.headSha});
  ...
  const webRuntimeEvidence=loadWebRuntimeHostRuntimeEvidence({path:process.env.UBERBOND_WEB_RUNTIME_HOST_RECEIPT_PATH||'',expectedSourceCommit:truthReceipt.headSha});
  const runtimeCutSetReport=applyRuntimeCutEvidenceOverlay({cutSetReport,sourceRepositoryEvidence,restartRecoveryEvidence:restartRuntimeEvidence,webRuntimeEvidence});
  const modelProviderEvidence=loadModelProviderRuntimeEvidence({path:process.env.UBERBOND_MODEL_PROVIDER_RECEIPT_PATH||'',expectedSourceCommit:truthReceipt.headSha});
  const modelCutSetReport=applyProviderCutEvidenceOverlay({cutSetReport:runtimeCutSetReport,modelProviderEvidence});
  const effectiveCutSetReport=applyRemainingCutEvidenceOverlay({cutSetReport:modelCutSetReport,evidenceByCut:remainingEvidence(truthReceipt.headSha)});
  const failed=runs.filter(r=>r.exitCode!==0);
  if(failed.length){
    const semanticRun=runs.find(r=>r.script==='scripts/semantic-requirement-tribunal.mjs');
    if(semanticRun?.exitCode!==0&&semanticTribunal){const handoff=compileTerminalSemanticRefusalHandoff({truthReceipt,semanticTribunal,runs,sinkAuthorityReport,cutSetReport:effectiveCutSetReport});if(handoff.status==='TERMINAL_REALIZATION_REFUSED')return persist({...handoff,generatedAt:new Date().toISOString(),generator:'scripts/terminal-realization.mjs'});}
