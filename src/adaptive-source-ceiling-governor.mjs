...
export function buildSourceCeilingPolicy({sourceId,mode='POLL',publishedLimit=null,publishedWindowSeconds=null,quotaUnitsPerDay=null,minDelayMs=250,maxConcurrency=1,continuous=false}={}){
 ...
 return {
  ...
  mode,
  publishedLimit:n(publishedLimit),
  publishedWindowSeconds:n(publishedWindowSeconds),
  quotaUnitsPerDay:n(quotaUnitsPerDay),
  minDelayMs:Math.max(0,Number(minDelayMs)||0),
  maxConcurrency:Math.max(1,Math.floor(Number(maxConcurrency)||1)),
  continuous:Boolean(continuous),
  authority:'PUBLIC_READ_RESEARCH_ONLY',
  laws:['NEVER_EVADE_RATE_LIMITS','HONOR_RETRY_AFTER_AND_RESET','USE_CONDITIONAL_REQUESTS_WHERE_SUPPORTED','CACHE_AND_DEDUPE','NO_AUTHORITY_FROM_INGESTION']
 };
}
...
export function nextSourceAction({policy,headers={},status=200,nowMs=Date.now(),usedQuotaUnitsToday=0,lastRequestAtMs=0}={}){
 ...
 const floor=Math.max(policy.minDelayMs||0,Math.max(0,(lastRequestAtMs||0)+(policy.minDelayMs||0)-nowMs));
...
export function buildAlwaysOnSensoriumPlan({overrides={}}={}){
 ...
 return {version:SOURCE_CEILING_GOVERNOR_VERSION,status:'ALWAYS_ON_SENSORIUM_PLAN_READY',lanes,goal:'MAXIMIZE_LAWFUL_USEFUL_SIGNAL_INGESTION_PER_UNIT_TIME_WITHOUT_LIMIT_EVASION',schedulerLaw:'RUN_24_7_WHILE_CAPACITY_EXISTS_SLEEP_EXACTLY_WHEN_PROVIDER_OR_POLITENESS_REQUIRES'};
}
