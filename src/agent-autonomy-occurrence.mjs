...
// Compiles a restart-stable concrete occurrence of a logical autonomy mission.
// occurrenceKey must come from a durable scheduler/event identity, never random
// process state. Reusing the same key is an idempotent retry. A new scheduled
// occurrence must use a new key.
export function compileAutonomyOccurrenceSession({
  occurrenceKey,
  missionKey = null,
  ...sessionInput
} = {}) {
...
export function compileAutonomyOccurrenceTaskIntent(input = {}) {
  ...
  if (!session?.ok || !text(session.occurrenceKey, 240) || !text(session.missionKey, 240)) {
