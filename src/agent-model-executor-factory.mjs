...
//
// Every provider stays DISABLED unless its credential and attributable cost
// evidence are present. Capability never creates authority and failover never
// hides which provider/model actually served a task.
...
const API_PROVIDERS = Object.freeze(['openai', 'anthropic', 'vercel-ai-gateway']);
...
export function pricingFrom(env, prefix) {
  ...
  if (!Number.isFinite(input) || input < 0 || !Number.isFinite(output) || output < 0 || !sourceRef || !verifiedAt) return null;
  const when = Date.parse(verifiedAt);
  if (!Number.isFinite(when)) return null;
  return { inputUsdPerMillion: input, outputUsdPerMillion: output, sourceRef, verifiedAt: new Date(when).toISOString() };
}

function apiProviderConfig(env, provider) {
  if (provider === 'openai') return {
    apiKey:String(env.OPENAI_API_KEY || ''), pricing:pricingFrom(env,'OPENAI'), enabled:env.OPENAI_AGENT_ENABLED === 'true'
  };
  if (provider === 'anthropic') return {
    apiKey:String(env.ANTHROPIC_API_KEY || ''), pricing:pricingFrom(env,'ANTHROPIC'), enabled:env.ANTHROPIC_AGENT_ENABLED === 'true'
  };
  return {
    apiKey:String(env.AI_GATEWAY_API_KEY || ''), pricing:pricingFrom(env,'AI_GATEWAY'), enabled:env.AI_GATEWAY_AGENT_ENABLED === 'true'
  };
}
...
export function createModelExecutorFactory({ env = process.env, sandboxIsolationReceipt = null } = {}) {
  return function modelExecutorFor(worker = {}) {
    ...
    if (provider === 'openai') {
      return createOpenAIAgentExecutor({ apiKey:config.apiKey, pricing:config.pricing, enabled:config.enabled, ...(worker.model ? { defaultModel: worker.model } : {}) });
    }
    if (provider === 'anthropic') {
      return createAnthropicAgentExecutor({ apiKey:config.apiKey, pricing:config.pricing, enabled:config.enabled, ...(worker.model ? { defaultModel: worker.model } : {}) });
    }
