...
const REGISTRY = Object.freeze([
  ...
  { id: 'opportunity-registry', name: 'Opportunity Registry (Business Genome + scoring tournament)', status: 'TEST_VERIFIED',
    dependencies: [], testRefs: ['tests/opportunity-registry.test.mjs', 'tests/opportunity-tournament.test.mjs'],
    productionReadiness: 'Pure/no I/O scoring plus a receipt-backed deterministic tournament over the 438-row canonical catalog. Usefulness still depends on real evidence being fed in over time.', notes: 'src/opportunity-registry.mjs, src/opportunity-tournament.mjs' },
  ...
  { id: 'genome-extraction', name: 'Signal-to-BusinessGenome extraction bridge', status: 'TEST_VERIFIED',
    ...
    productionReadiness: 'Local-only; converts already-accepted signals (from either surviving ingestion pipeline\'s accepted-signal shape) into a scoreOpportunity-ready candidate, keeps the weakest evidence tier honest, and is wired through prometheus.genome.extract with one compact auditLog receipt. No network or promotion authority.',
    notes: 'src/genome-extraction.mjs and prometheus.genome.extract in src/job-handlers.mjs. Kept from this session\'s side of Pair 1 (see docs/PROMETHEUS_PARALLEL_SPINE_RECONCILIATION.md) -- the only bridge from a normalized signal into opportunity-registry.mjs.' },
  ...
  { id: 'prometheus-economic-spine', name: 'Prometheus signal-to-offer economic spine', status: 'TEST_VERIFIED',
    ...
    testRefs: ['tests/prometheus-economic-spine.test.mjs', 'tests/commercial-reconciliation.test.mjs'],
    productionReadiness: 'Local-only composition of canonical signal, genome, opportunity score, offer, and dry-run experiment packet. The signal→BusinessGenome→tournament reconciliation writes only bounded auditLog receipts when explicitly persisted. No commercial claim or external effect.',
    notes: 'src/prometheus-economic-spine.mjs and src/commercial-reconciliation.mjs via prometheus.commercial.reconcile' },
  { id: 'approved-source-rehearsal', name: 'Catalog-approved buyer-signal reconciliation rehearsal', status: 'TEST_VERIFIED',
    dependencies: ['market-signal-registry', 'genome-extraction', 'opportunity-registry'], testRefs: ['tests/approved-source-rehearsal.test.mjs'],
    productionReadiness: 'Local-only and explicit-allowlist only. Replays canonical BUYER_SIGNAL observations through the existing reconciliation path, preserves UNVERIFIED source state without a separate receipt reference, and cannot contact, spend, deploy, promote, or claim revenue.',
    notes: 'src/approved-source-rehearsal.mjs via prometheus.commercial.approved_source_rehearsal' },
  ...
  { id: 'commercial-first-payment-gate', name: 'Owner-reviewable first-payment and seven-day gate packet', status: 'TEST_VERIFIED',
    dependencies: ['commercial-experiment-compiler', 'commercial-outcome-lineage', 'adapter-contracts'], testRefs: ['tests/commercial-first-payment-packet.test.mjs'],
    productionReadiness: 'Composes the catalog seven-day experiment with legal owner attestation, provider access, payment-truth, and outcome-lineage gates. It is no-contact/no-checkout preparation only; first payment remains external proof required.',
    notes: 'src/commercial-first-payment-packet.mjs via prometheus.commercial.first_payment_packet' },
  ...
  { id: 'provider-adapter-contract', name: 'Provider adapter contract and Icemail/Mailforge/AgentMail infrastructure clients', status: 'TEST_VERIFIED',
    dependencies: [], testRefs: ['tests/domain-mailbox-readiness.test.mjs', 'tests/provider-http-adapters.test.mjs', 'tests/agentmail-adapter.test.mjs'],
    productionReadiness: 'Documented Icemail, Mailforge and AgentMail routes are implemented behind injectable HTTP adapters with secret rejection, bounded reads, mutation approval, idempotency and unknown-outcome recovery. AgentMail target-plan ceilings remain unobserved until authenticated receipts; no provider credential is configured on this branch, so no live provider state is claimed.',
    notes: 'src/provider-adapter-contract.mjs, src/provider-http-adapters.mjs, src/agentmail-adapter.mjs' },
  { id: 'ubermail-agent-api', name: 'Owned UberMail Agent API (private AgentMail-like control plane)', status: 'TEST_VERIFIED',
    dependencies: [], testRefs: ['tests/ubermail-agent-api.test.mjs', 'tests/ubermail-agent-http.test.mjs', 'tests/ubermail-uberdoso-bridge.test.mjs', 'tests/ubermail-foundry.test.mjs'],
    productionReadiness: 'Clean-room private mailbox API is deterministically tested across scoped bearer keys, pods/domains/subdomains/inboxes, messages/replies/forwards, searches, threads, reply/forward drafts, scheduling, attachments, scoped webhooks with write-only headers, allow/block lists, inbound ingestion, metrics, realtime event streaming, durable state, the existing UberMail Foundry/JIT/capacity organs, and an authority-preserving UberDoso bridge. It does not claim a live Postal/Stalwart host, DNS, reputation, egress capacity, or provider acceptance.',
    notes: 'src/ubermail-agent-api.mjs, src/ubermail-agent-http.mjs, src/ubermail-file-repository.mjs, src/ubermail-uberdoso-bridge.mjs, src/ubermail-foundry.mjs, src/ubermail-jit-provisioning.mjs, src/ubermail-capacity-exchange.mjs' },
  { id: 'mailhub-control-plane', name: 'UberBond MailHub provider-neutral control plane', status: 'TEST_VERIFIED',
    dependencies: ['provider-adapter-contract', 'sending-domain-registry', 'sending-mailbox-registry', 'domain-mailbox-gate'], testRefs: ['tests/mailhub-control-plane.test.mjs', 'tests/provider-infrastructure-events.test.mjs'],
    productionReadiness: 'Compiles provisioning plans, reconciles provider inventory, ranks only provider-confirmed sender capacity, and folds authenticated infrastructure events monotonically. It does not manufacture accounts, reputation, capacity, customers or revenue.',
    notes: 'src/mailhub-control-plane.mjs, src/provider-infrastructure-events.mjs' },
  { id: 'warmup-orchestrator', name: 'Native provider warm-up orchestration', status: 'TEST_VERIFIED',
    ...
    productionReadiness: 'Decision logic only calls a provider adapter\'s own capabilities and folds its real response; with no provider credential configured, current activation remains BLOCKED_PROVIDER_AUTH. A configured provider still needs provider-documented payloads and explicit owner approval.',
  ...
  { id: 'domain-mailbox-gate', name: 'Domain/mailbox deny-only pre-check (composes with Deliverability Guard/V9)', status: 'TEST_VERIFIED',
    dependencies: ['sending-domain-registry', 'sending-mailbox-registry', 'deliverability-guard'], testRefs: ['tests/domain-mailbox-readiness.test.mjs', 'tests/pipeline-domain-mailbox-gate.test.mjs'],
    productionReadiness: 'Can only DENY or REVIEW_REQUIRED; its pass state is explicitly named NOT_BLOCKED_BY_DOMAIN_MAILBOX_GATE, never ALLOW, so it cannot be mistaken for final authorization. Pipeline.maybeSend consults it before reservation/provider dispatch when outbound.domainMailboxGateRequired is true (the production default), with explicit account-to-domain/mailbox/workspace linkage, fresh DNS, authenticated warm-up, provider cap, and owner authorization required.',
  ...
  { id: 'commercial-opportunity-catalog', name: 'Evidence-labeled commercial opportunity catalog (438 records)', status: 'TEST_VERIFIED',
    dependencies: ['task-universe-engine'], testRefs: ['tests/commercial-opportunity-catalog.test.mjs'],
    productionReadiness: 'Merged from PR #27 and reconciled here. 3 immediate finalists carry dated public buyer-signal evidence; 435 records are explicitly HYPOTHESIS/RESEARCH_ONLY -- no buyer, payment, retention, or profitability proof exists for those. Explicit taxonomy rows are projected into the same canonical field-complete registry; UNRESOLVED values remain visible. LOCAL_PREPARATION_ONLY; zero provider calls.',
