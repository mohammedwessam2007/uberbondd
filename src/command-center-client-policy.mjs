export const COMMAND_CENTER_CLIENT_POLICY_VERSION = 'uberbond.command-center-client-policy.v1';
...
export function classifyCachedSnapshot(cache, { now = Date.now(), maxAgeMs = COMMAND_CENTER_MAX_CACHE_AGE_MS } = {}) {
  ...
  if (!Number.isFinite(stored) || !Number.isFinite(current)) return { state: 'INVALID', ageMs: null };
...
export function classifyLiveStatus(status) {
