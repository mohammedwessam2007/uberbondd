...
function historyProjection(entry) {
  return {
    ...
    sourceNodeId: entry.event.sourceNodeId,
    subjectType: entry.event.subjectType,
    subjectId: entry.event.subjectId,
    summary: entry.event.summary,
    truthClass: entry.event.truthClass,
    observedAt: entry.event.observedAt,
    evidenceRefs: [...entry.event.evidenceRefs],
    parentEventIds: [...entry.event.parentEventIds]
  };
}
...
export function retrieveRelevantCognitiveHistory({ entries = [], mission, maxEvents = 24 } = {}) {
  ...
  return {
    ...
    journalTipDigest: verified.tipDigest,
    events: selected.map(row => historyProjection(row.entry)),
    businessEffectAuthority: 'NONE',
    externalEffectAuthority: 'NONE',
    externalEffectLedger: zeroEffects()
  };
...
export function compileContextMount({ doctorResult, journalEntries = [], mission = null, maxHistoricalEvents = 24, recompiledFromStale = false } = {}) {
  if (!doctorResult?.ok || doctorResult.status !== 'CONTEXT_CURRENT__MISSION_CONTEXT_READY') {
    return fail(['current-context-doctor-result-required'], 'CONTEXT_MOUNT_REFUSED');
  }
  ...
  const mount = {
    schemaVersion: CONTEXT_MOUNT_SCHEMA_VERSION,
    contextAbiVersion: doctorResult.capsule.contextAbiVersion,
    sourceCommit: doctorResult.sourceCommit,
    brainstateId: doctorResult.brainstateId,
    contextIdentityDigest: doctorResult.contextIdentityDigest,
    missionContextId: doctorResult.missionContext.missionContextId,
    mission: missionText,
    recompiledFromStale: recompiledFromStale === true,
    ...
    laws: {
      zeroRetelling: doctorResult.capsule.laws.zeroRetellingLaw,
      staleContext: doctorResult.capsule.laws.staleContextLaw,
  ...
  return {
    ok: true,
    policyVersion: CONTEXT_HISTORY_RETRIEVAL_POLICY_VERSION,
    status: recompiledFromStale ? 'CONTEXT_MOUNT_READY__STALE_CACHE_RECOMPILED' : 'CONTEXT_MOUNT_READY',
    mount,
    externalEffectLedger: zeroEffects()
  };
}
