...
export const CONTEXT_JOURNAL_RUNTIME_VERSION='context-journal-runtime-1.2.0';
...
function readTail(file){if(!fs.existsSync(file))return[];try{const st=fs.lstatSync(file);if(!st.isFile()||st.isSymbolicLink()||st.size>MAX_JSON)throw new Error('unsafe-tail-file');const raw=fs.readFileSync(file,'utf8');return raw.split('\n').filter(Boolean).map(line=>JSON.parse(line));}catch{return null;}}
...
export function compileContextLayeredStoreDescriptor({archiveManifestId,archiveEntryCount,archiveTipDigest,legacyJournalPath=null}={}){
  const descriptor={schemaVersion:CONTEXT_LAYERED_STORE_SCHEMA,archiveManifestId,archiveEntryCount,archiveTipDigest,legacyJournalPath:legacyJournalPath?path.resolve(legacyJournalPath):null,tailFormat:'GLOBAL_COGNITIVE_JOURNAL_ENTRY_V1',consequenceAuthority:'NONE',businessEffectAuthority:'NONE',externalEffectAuthority:'NONE'};
  descriptor.descriptorId=digest(descriptorPayload(descriptor));return descriptor;
}
...
export function appendContextJournalRuntime({journalPath,layeredStorePath=null,compiledEvent}={}){
  ...
  try{try{handle=fs.openSync(lock,'wx',0o600);}catch(error){if(error?.code==='EEXIST')return fail(['journal-writer-lock-held'],'COGNITIVE_JOURNAL_BUSY');throw error;}
    const check=readContextJournalRuntime({journalPath,layeredStorePath:root});if(!check.ok||check.entryCount!==current.entryCount||check.tipDigest!==current.tipDigest||check.descriptorId!==descriptor.descriptorId)return fail(['layered-journal-changed-during-append'],'CONTEXT_JOURNAL_RACE_REFUSED');
    const fd=fs.openSync(tailPath,'a',0o600);try{fs.writeSync(fd,`${JSON.stringify(global.entry)}\n`,null,'utf8');fs.fsyncSync(fd);}finally{fs.closeSync(fd);}fs.chmodSync(tailPath,0o600);
