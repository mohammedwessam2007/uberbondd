...
import { verifyContextSchemaCompatibility } from './context-schema-compatibility-verify.mjs';

export const CONTEXT_REPLICATION_BUNDLE_POLICY_VERSION='context-replication-bundle-1.2.0';
...
export function compileContextReplicationBundle({brainstate,manifest,segments}={}){
  ...
  const compatibilityCheck=verifyContextSchemaCompatibility(compatibility.receipt);
  if(!compatibilityCheck.ok)return fail(['verified-context-schema-compatibility-required',...(compatibilityCheck.reasonCodes||[])]);
  ...
  return{ok:true,policyVersion:CONTEXT_REPLICATION_BUNDLE_POLICY_VERSION,status:'CONTEXT_REPLICATION_BUNDLE_READY',bundle,schemaCompatibility:compatibility.receipt,schemaCompatibilityVerified:true,businessEffectAuthority:'NONE',externalEffectAuthority:'NONE',externalEffectLedger:zeroEffects()};
}
