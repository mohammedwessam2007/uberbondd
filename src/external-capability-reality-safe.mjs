...
export const SAFE_EXTERNAL_CAPABILITY_GAPS=Object.freeze([
  'autonomous-exploit-verification-runtime',
...
case 'autonomous-exploit-verification-runtime': if(r.targetAuthorized!==true)reasons.push('owned-or-explicitly-authorized-target-required');if(r.runtimeExecuted!==true)reasons.push('real-runtime-execution-required');if(r.exploitWitnessObserved!==true)reasons.push('exploit-witness-required');if(r.boundedNonDestructive!==true)reasons.push('bounded-non-destructive-verification-required');if(!text(r.targetIdentityRef,1000)||!text(r.runtimeReceiptRef,1000))reasons.push('target-and-runtime-receipt-required');break;
...
export function evaluateSafeExternalCapabilityReality({receipts=[],now=new Date()}={}){
  ...
  return envelope({ok:true,status:openIds.length?'SAFE_EXTERNAL_CAPABILITY_REALITY_PARTIAL':'SAFE_EXTERNAL_CAPABILITY_REALITY_COMPLETE',closedCount:closedIds.length,openCount:openIds.length,closedIds,openIds,verdicts,verdictDigest:`sha256:${hash(verdicts)}`,truthBoundary:'ONLY CUT-SPECIFIC FRESH NON-SYNTHETIC REALITY RECEIPTS MAY CLOSE THESE SAFE EXTERNAL CAPABILITY GAPS. OWNED-TARGET SECURITY EVIDENCE MUST COME FROM A REAL BOUNDED NON-DESTRUCTIVE RUNTIME. THIS VERIFIER CREATES NO PROVIDER, OWNER, PAYMENT, DEPLOYMENT, MESSAGE, SECURITY, OR IDENTITY EVIDENCE.'});
}
