...
export function compileTopMillionWormholePriors({topK=100}={}){
  if(!Number.isSafeInteger(topK)||topK<1||topK>COMPILED_MAX_EXACT_TOP_K){
    return {ok:false,status:'COMPILED_PRIOR_SEARCH_REFUSED',reasonCodes:['top-k-outside-exact-compiled-envelope'],exactTopKLimit:COMPILED_MAX_EXACT_TOP_K,businessEffectAuthority:'NONE',externalEffectAuthority:'NONE',externalEffectLedger:zero()};
  }
  const rows=[];
      ...
      for(const operatorId of HIGH_OPERATOR_IDS){
        for(const substrateId of LOW_COST_SUBSTRATE_IDS){
          for(const verifierId of STRONG_VERIFIER_IDS){
            ...
            rows.push({...renderMillionWormholeCandidate(index),staticPrior:.98});
          }
        }
      }
    }
