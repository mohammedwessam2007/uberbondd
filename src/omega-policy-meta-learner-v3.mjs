...
export const OMEGA_POLICY_META_LEARNER_V3_VERSION='uberbond.omega-policy-meta-learner.v3';
...
function supportEnvelope(examples,keys){
  const out={};
  for(const key of keys){ const values=examples.map(e=>Number(e.features[key]??0)); out[key]={min:Math.min(...values),max:Math.max(...values)}; }
  return out;
}
...
export function trainCalibratedMetaCrystal({sourceTasks=[],policyBudget=5000,distanceMultiplier=1.5}={}){
  ...
  const loo=examples.map((e,i)=>nearestDistance(e.features,examples,keys,scales,i));
  const finite=loo.filter(Number.isFinite).sort((a,b)=>a-b); if(!finite.length) return fail('calibration-distances-required');
  const maxLoo=finite.at(-1), p95=finite[Math.min(finite.length-1,Math.floor(0.95*(finite.length-1)))];
  const threshold=Math.max(maxLoo,p95)*distanceMultiplier;
  const envelopeSupport=supportEnvelope(examples,keys);
  const core={baseCrystalHash:base.crystal.crystalHash,featureKeys:keys,scales,examples,sourceDomains:base.crystal.sourceDomains,k:base.crystal.k,policyBudget,calibration:{distanceMultiplier,maxLeaveOneOutDistance:maxLoo,p95LeaveOneOutDistance:p95,oodDistanceThreshold:threshold,supportEnvelope:envelopeSupport,typeKeys:TYPE_KEYS}};
  return envelope({ok:true,status:'OMEGA_CALIBRATED_META_CRYSTAL_TRAINED',version:OMEGA_POLICY_META_LEARNER_V3_VERSION,crystal:{...core,crystalHash:digest(core),containsAnswers:false,containsTargetTasks:false,promotionAuthority:'NONE'},training:{...base.training,calibrationDistances:loo},truthBoundary:'Calibration is derived only from source structural features. Abstention reduces unsupported transfer risk but cannot prove that every in-threshold target is safe.'});
}

function supportViolations(features,calibration){
  const violations=[]; const env=calibration.supportEnvelope;
  ...
  for(const [key,range] of Object.entries(env)){
    const value=Number(features[key]??0); const span=Math.max(1e-12,range.max-range.min); const margin=span===1e-12?Math.max(0.05,Math.abs(range.max)*0.25):span*0.5;
    if(value<range.min-margin||value>range.max+margin) violations.push(`outside-support:${key}`);
  }
...
export function selectCalibratedPolicy({crystal,targetTask}={}){
  ...
  const features=extractStructuralFeatures(task); const d=nearestDistance(features,crystal.examples,crystal.featureKeys,crystal.scales);
  const violations=supportViolations(features,crystal.calibration); const distanceOod=!Number.isFinite(d)||d>crystal.calibration.oodDistanceThreshold;
  if(distanceOod||violations.length){
    const core={targetFeatureHash:digest(features),nearestDistance:d,threshold:crystal.calibration.oodDistanceThreshold,violations};
    return envelope({ok:true,status:'OMEGA_META_POLICY_ABSTAINED',version:OMEGA_POLICY_META_LEARNER_V3_VERSION,abstained:true,selectedPolicy:'INPUT_ORDER',confidence:'OOD_FALLBACK',ood:{...core,oodHash:digest(core)},truthBoundary:'INPUT_ORDER is used only as an abstention fallback. It is not credited as transferred intelligence.'});
  }
  const v=vector(features,crystal.featureKeys,crystal.scales);
  const neighbors=crystal.examples.map((e,i)=>({i,policy:e.bestPolicy,distance:distance(v,vector(e.features,crystal.featureKeys,crystal.scales))})).sort((a,b)=>a.distance-b.distance||a.i-b.i).slice(0,Math.max(1,Math.min(crystal.k??3,crystal.examples.length)));
  ...
  const core={targetFeatureHash:digest(features),nearestDistance:d,threshold:crystal.calibration.oodDistanceThreshold,selectedPolicy:selected,neighborPolicies:neighbors.map(n=>n.policy)};
  return envelope({ok:true,status:'OMEGA_META_POLICY_TRANSFER_SELECTED',version:OMEGA_POLICY_META_LEARNER_V3_VERSION,abstained:false,selectedPolicy:selected,confidence:'IN_SUPPORT',adapter:{...core,adapterHash:digest(core),answerFree:true,usesTargetPortfolioRuns:false,usesIdentifiers:false},neighbors});
}

export function evaluateCalibratedTransfer({crystal,targetTasks=[]}={}){
  if(!crystal?.crystalHash) return fail('calibrated-crystal-required');
  ...
  for(const raw of targetTasks){ const task=normalizeProblem(raw); if(!task) return fail('invalid-target-task'); const choice=selectCalibratedPolicy({crystal,targetTask:task}); if(!choice.ok)return choice; const cold=solveProblem({problem:task,policy:'INPUT_ORDER'}); const run=choice.abstained?cold:solveProblem({problem:task,policy:choice.selectedPolicy}); if(!cold.ok||!run.ok||!cold.verifier?.valid||!run.verifier?.valid)return fail('target-solve-failed'); const cw=work(cold),tw=work(run);coldWork+=cw;executionWork+=tw;if(choice.abstained)abstentions++;else transfers++;rows.push({taskHash:digest(task),domain:task.domain,selectedPolicy:choice.selectedPolicy,abstained:choice.abstained,confidence:choice.confidence,coldWork:cw,executionWork:tw,reduction:cw-tw,ood:choice.ood??null}); }
  return envelope({ok:true,status:'OMEGA_CALIBRATED_TRANSFER_EVALUATED',version:OMEGA_POLICY_META_LEARNER_V3_VERSION,receipt:{targetCount:rows.length,coldWork,executionWork,reduction:coldWork-executionWork,reductionFraction:coldWork?(coldWork-executionWork)/coldWork:0,abstentions,transfers,improvedTasks:rows.filter(r=>r.reduction>0).length,worsenedTasks:rows.filter(r=>r.reduction<0).length,tiedTasks:rows.filter(r=>r.reduction===0).length,rows,catastrophicNegativeTransferPrevented:rows.filter(r=>r.abstained).every(r=>r.executionWork===r.coldWork)},truthBoundary:'Abstention converts detected out-of-support tasks to baseline execution. Success here demonstrates calibrated failure handling in this bounded representation, not universal uncertainty calibration.'});
}
