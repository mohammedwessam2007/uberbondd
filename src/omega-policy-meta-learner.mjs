...
export function selectPolicyFromMetaCrystal({crystal,targetTask}={}){
  ...
  const neighbors=crystal.examples.map((e,i)=>({i,policy:e.bestPolicy,distance:distance(v,vector(e.features,crystal.scales)),observedSavings:e.observedSavings,observedSavingsFraction:e.observedSavingsFraction})).sort((a,b)=>a.distance-b.distance||a.i-b.i).slice(0,Math.max(1,Math.min(crystal.k??3,crystal.examples.length)));
