...
export function validateRemainingCutEvidence({receipt,expectedSourceCommit,expectedCutId,evidenceRef}={}){
  const expected=String(expectedSourceCommit||'').trim().toLowerCase(),cutId=String(expectedCutId||'').trim().toUpperCase(),reasons=[];const hasReceipt=Boolean(receipt&&typeof receipt==='object'&&!Array.isArray(receipt));
  if(!SHA40.test(expected))reasons.push('exact-expected-source-commit-required');if(!REMAINING_CUT_IDS.includes(cutId))reasons.push('recognized-expected-cut-required');if(!ref(evidenceRef))reasons.push('external-evidence-reference-required');if(!verifyRemainingCutEvidenceIntegrity(receipt))reasons.push('canonical-remaining-cut-receipt-required');if(hasReceipt&&String(receipt.sourceCommit||'').toLowerCase()!==expected)reasons.push('remaining-cut-source-mismatch');if(hasReceipt&&receipt.cutId!==cutId)reasons.push('remaining-cut-id-mismatch');const accepted=reasons.length===0;
