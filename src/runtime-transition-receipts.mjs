...
export const RUNTIME_TRANSITION_RECEIPTS_VERSION='uberbond.runtime-transition-receipts.v1.0.1';
...
const normIdentity=(v,max=500)=>text(v,max)?.toLowerCase()||null;
...
function common(input,kind){
  ...
  if(runtimeIdentity&&verifierIdentity&&normIdentity(runtimeIdentity)===normIdentity(verifierIdentity)) reasons.push('runtime-cannot-self-verify');
...
export function compileCutoverRollbackReceipt(input={}){
  ...
  if(!fromRuntimeIdentity||!toRuntimeIdentity||normIdentity(fromRuntimeIdentity)===normIdentity(toRuntimeIdentity)) r.push('distinct-cutover-runtime-identities-required');
  if(c.runtimeIdentity&&toRuntimeIdentity&&normIdentity(c.runtimeIdentity)!==normIdentity(toRuntimeIdentity)) r.push('observed-runtime-must-be-cutover-target');
  ...
  if(normIdentity(input.rollbackRuntimeIdentity)!==normIdentity(fromRuntimeIdentity)) r.push('rollback-must-return-to-source-runtime');
  if(input.duplicateExternalEffects!==0) r.push('zero-duplicate-external-effects-required');
  if(input.uncertainEffectBlindReplayObserved===true) r.push('uncertain-effect-blind-replay-prohibited');
...
export function compileProviderLossReceipt(input={}){
  ...
  if(!failedProvider||!alternateProvider||normIdentity(failedProvider,160)===normIdentity(alternateProvider,160)) r.push('distinct-provider-boundary-required');
