...
function coreOf(request = {}) {
  return {
    ...
    promotionClass: request.promotionClass,
...
export function compileSovereignReleaseRequest({ mergeReceipt, promotionReceipt } = {}) {
  const m = promotionReceipt || mergeReceipt || {};
  const reasons = [];
  const isGithub = m.ok === true && m.status === GITHUB_STATUS;
  const isLocal = m.ok === true && m.status === LOCAL_STATUS;
  const sourceCommit = text(isLocal ? m.promotionCommitSha : m.mergeCommitSha, 80).toLowerCase();
  const candidateHeadSha = text(m.headSha, 80).toLowerCase();
  const priorMainSha = text(m.priorMainSha, 80).toLowerCase();
  const prNumber = isGithub ? Number(m.prNumber || 0) : null;
  const promotionClass = isLocal ? 'LOCAL_MAIN_FAST_FORWARD' : 'GITHUB_PR_MERGE';
  const promotionRef = isLocal ? text(m.promotionRef, 200) : `pull-request:${prNumber}`;
  const changeSetId = text(m.changeSetId, 100);
  const selfMaintenanceReceiptId = text(m.receiptId, 100);
  if (!isGithub && !isLocal) reasons.push('verified-autonomous-promotion-receipt-required');
  if (!SHA40.test(sourceCommit)) reasons.push('exact-promotion-commit-required');
  ...
  if (String(m.signingAuthority || 'NONE').toUpperCase() !== 'NONE') reasons.push('promotion-receipt-must-not-carry-signing-authority');
  if (String(m.deploymentAuthority || '').toUpperCase() !== 'NONE') reasons.push('promotion-receipt-must-not-carry-deployment-authority');
  if (String(m.businessEffectAuthority || '').toUpperCase() !== 'NONE') reasons.push('promotion-receipt-must-not-carry-business-effect-authority');
  if (String(m.externalEffectAuthority || '').toUpperCase() !== 'NONE') reasons.push('promotion-receipt-must-not-carry-external-effect-authority');
  ...
  const core = {
    ...
    independentVerificationClass: isLocal
      ? 'LOCAL_PROMOTION_GOVERNOR_CONFIRMED_VERIFIER_RECEIPT_AND_FIXED_SYNTAX_DETERMINISTIC_TRIBUNAL'
      : 'MERGE_GOVERNOR_CONFIRMED_EXACT_HEAD_SYNTAX_AND_DETERMINISTIC_TRIBUNAL',
...
export function verifySovereignReleaseRequest(request = {}) {
  ...
  const declaredClass = text(request.promotionClass, 80);
  const legacyGithub = !declaredClass && Number.isSafeInteger(Number(request.prNumber)) && Number(request.prNumber) > 0;
  const github = declaredClass === 'GITHUB_PR_MERGE' || legacyGithub;
  const local = declaredClass === 'LOCAL_MAIN_FAST_FORWARD';
