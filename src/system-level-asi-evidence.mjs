...
export const SYSTEM_LEVEL_ASI_EVIDENCE_VERSION = 'uberbond.system-level-asi-evidence.v1';
...
export const CANONICAL_ASI_DIMENSIONS = Object.freeze([
  ...
  'learning from sparse evidence',
  'capability acquisition',
  'autonomous recovery',
  'self-improvement',
  'tool invention',
  'adversarial robustness',
  'calibrated refusal and ignorance detection'
]);
...
const txt=(v,n=500)=>typeof v==='string'&&v.trim()&&v.trim().length<=n?v.trim():null;
...
function normalizeDimension(row, nowMs, maxAgeMs) {
  ...
  const verifierId=txt(row?.verifierId,200);
  const verifierLineageRef=txt(row?.verifierLineageRef,500);
  const taskPopulationHash=txt(row?.taskPopulationHash,64)?.toLowerCase();
  const observed=instant(row?.observedAt);
  const c13=row?.compoundEvaluation;
...
/**
 * C21 system-level evidence tribunal.
 * This aggregates already-earned C13 evidence across the frozen 20-dimension
 * taxonomy. It never establishes ASI, promotes code, allocates resources, or
 * executes self-modification. Its job is anti-cherry-picking and stage control.
 */
export function evaluateSystemLevelAsiEvidence({
  candidateId=null,
  candidateRevision=null,
  dimensions=[],
  observedAt=null,
  maxEvidenceAgeMs=30*24*60*60*1000,
  minimumPartialDimensions=12,
  ...
  const unexpected=rows.filter(r=>!CANONICAL_ASI_DIMENSIONS.includes(r.dimension)).map(r=>r.dimension);
  if(unexpected.length) return fail(['only-canonical-c21-dimensions-may-count'],{unexpectedDimensions:unexpected});
  ...
  const verifierIds=new Set(rows.map(r=>r.verifierId));
  const verifierLineages=new Set(rows.map(r=>r.verifierLineageRef));
  const independentEnough=verifierIds.size>=minimumIndependentVerifierIdentities&&verifierLineages.size>=minimumIndependentVerifierIdentities;
  const covered=new Set(rows.map(r=>r.dimension));
  const missing=CANONICAL_ASI_DIMENSIONS.filter(d=>!covered.has(d));
  const missingCritical=CRITICAL_DIMENSIONS.filter(d=>!covered.has(d));
  const contaminationBounded=rows.every(r=>['CLEAN','BOUNDED_DISCLOSED'].includes(r.contaminationStatus));

  let evidenceStage='SYSTEM_LEVEL_ASI_EVIDENCE_NOT_ESTABLISHED';
  if(rows.length===CANONICAL_ASI_DIMENSIONS.length&&missingCritical.length===0&&independentEnough&&contaminationBounded) {
    evidenceStage='SYSTEM_LEVEL_ASI_EVIDENCE_STRONG_WITHIN_DEFINED_SCOPE';
  } else if(rows.length>=minimumPartialDimensions&&missingCritical.length===0&&independentEnough&&contaminationBounded) {
    evidenceStage='SYSTEM_LEVEL_ASI_EVIDENCE_PARTIAL';
  }

  const canonicalRows=[...rows].sort((a,b)=>a.dimension.localeCompare(b.dimension));
  const evidenceDigest=hash({version:SYSTEM_LEVEL_ASI_EVIDENCE_VERSION,candidateId:id,candidateRevision:revision,canonicalDimensions:CANONICAL_ASI_DIMENSIONS,rows:canonicalRows,policy:{minimumPartialDimensions,minimumIndependentVerifierIdentities,maxEvidenceAgeMs}});
  return {
    ok:true,
    version:SYSTEM_LEVEL_ASI_EVIDENCE_VERSION,
    status:'C21_SYSTEM_LEVEL_COMPARATIVE_EVIDENCE_COMPILED',
    evidenceStage,
    asiStatus:SYSTEM_LEVEL_ASI_STATUS,
    candidate:{candidateId:id,candidateRevision:revision},
    evidenceDigest,
    counts:{canonicalDimensions:CANONICAL_ASI_DIMENSIONS.length,evidencedDimensions:rows.length,missingDimensions:missing.length,independentVerifierIdentities:verifierIds.size,independentVerifierLineages:verifierLineages.size},
    missingDimensions:missing,
    missingCriticalDimensions:missingCritical,
    dimensionEvidence:canonicalRows,
    truthBoundary:'C21_CAN_ONLY_STAGE_EVIDENCE_WITHIN_DEFINED_OBSERVED_SCOPE__IT_CAN_NEVER_DECLARE_OR_ESTABLISH_ASI',
    generalityBoundary:'NO_EVIDENCE_OUTSIDE_THE_MATERIALIZED_TASK_POPULATIONS_MAY_BE_INFERRED',
