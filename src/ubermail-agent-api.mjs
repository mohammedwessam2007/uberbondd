    ...
    if (typeof webhookDispatcher === 'function') {
      for (const hook of targets) {
        ...
        try {
          ...
          await webhookDispatcher({ webhook: publicWebhook(hook), event: eventPayload(event), signature, body });
  ...
  const api = {
    ...
    async createApiKey({ auth, name = '', type = 'bearer', permissions = DEFAULT_PERMISSIONS, podId = null, inboxId = null, expiresAt = null, idempotencyKey = '' } = {}) {
      return mutate(async state => {
        ...
        const keyType=text(type,40).toLowerCase(); if(!['bearer','public_key'].includes(keyType))throw new UberMailError('api-key-type-invalid',400);
        const secret = (keyType==='public_key'?'ubm_pk_':'ubm_') + crypto.randomBytes(24).toString('base64url');
    ...
    async listApiKeys({ auth, type = '', limit = 100, pageToken = '' } = {}) {
      return mutate(async state => {
        await authContext(state, auth, 'api_key_read');
        let rows = Object.values(state.apiKeys).filter(row => !type || row.type === type).map(publicApiKey);
    ...
    async createDomain({ auth, domain, podId=null, clientId='', metadata={}, idempotencyKey='' }={}) { return mutate(async state=>{ await authContext(state,auth,'domain_create',{podId}); const prior=replay(state,idempotencyKey); if(prior)return prior; const name=text(domain,253).toLowerCase(); if(!/^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}$/i.test(name)) throw new UberMailError('domain-invalid',400); if(podId)requirePod(state,podId); const existing=Object.values(state.domains).find(r=>r.domain===name); if(existing)return existing; const row={domain_id:random('domain'),domain:name,pod_id:podId||null,client_id:text(clientId,300)||null,metadata:clone(safeObject(metadata)),status:'pending',records:[],created_at:at(),updated_at:at()}; state.domains[row.domain_id]=row; await emit(state,'domain.created',row); return remember(state,idempotencyKey,clone(row)); }); },
    async listDomains({ auth, limit=100,pageToken='',ascending=false }={}) { return mutate(async state=>{ await authContext(state,auth,'domain_read'); let rows=Object.values(state.domains).map(clone); rows.sort((a,b)=>String(a.created_at).localeCompare(String(b.created_at))*(ascending?1:-1)); const out=page(rows,{limit,pageToken}); return{count:out.count,domains:out.items,limit:out.limit,next_page_token:out.next_page_token}; }); },
    async getDomain({ auth,domainId }={}) { return mutate(async state=>{ await authContext(state,auth,'domain_read'); return clone(requireDomain(state,domainId)); }); },
    async updateDomain({ auth,domainId,metadata }={}) { return mutate(async state=>{ await authContext(state,auth,'domain_update'); const row=requireDomain(state,domainId); if(metadata!==undefined)row.metadata=clone(safeObject(metadata)); row.updated_at=at(); return clone(row); }); },
    async verifyDomain({ auth,domainId,idempotencyKey='' }={}) { return mutate(async state=>{ await authContext(state,auth,'domain_update'); const prior=replay(state,idempotencyKey); if(prior)return prior; const row=requireDomain(state,domainId); if(typeof domainVerifier!=='function') throw new UberMailError('domain-verifier-not-configured',503); const evidence=await domainVerifier({domain:row.domain,domainId:row.domain_id,records:clone(row.records)}); row.records=array(evidence?.records).map(clone); row.status=evidence?.verified===true?'verified':'pending'; row.verified_at=evidence?.verified===true?at():null; row.updated_at=at(); if(row.status==='verified')await emit(state,'domain.verified',row); return remember(state,idempotencyKey,clone(row)); }); },
    async deleteDomain({ auth,domainId }={}) { return mutate(async state=>{ await authContext(state,auth,'domain_delete'); const row=requireDomain(state,domainId); if(Object.values(state.inboxes).some(i=>i.domain_id===domainId)) throw new UberMailError('domain-in-use',409); delete state.domains[row.domain_id]; await emit(state,'domain.deleted',{domain_id:row.domain_id,domain:row.domain}); return{deleted:true,domain_id:row.domain_id}; }); },

    async createInbox({ auth, username='',domain='',displayName='',clientId='',metadata={},podId=null,idempotencyKey='' }={}) { return mutate(async state=>{ await authContext(state,auth,'inbox_create',{podId}); const prior=replay(state,idempotencyKey); if(prior)return prior; if(podId)requirePod(state,podId); const local=text(username,64).toLowerCase()||crypto.randomBytes(5).toString('hex'); if(!/^[a-z0-9](?:[a-z0-9._+-]{0,62}[a-z0-9])?$/.test(local)) throw new UberMailError('username-invalid',400); let domainName=text(domain,253).toLowerCase()||'uberbond.local'; let domainRow=Object.values(state.domains).find(r=>r.domain===domainName)||null; if(domain && (!domainRow||domainRow.status!=='verified')) throw new UberMailError('verified-domain-required',409); const address=email(local+'@'+domainName,'inbox'); if(Object.values(state.inboxes).some(r=>r.email===address)) throw new UberMailError('inbox-conflict',409); if(clientId){const found=Object.values(state.inboxes).find(r=>r.client_id===clientId); if(found)return found;} const row={pod_id:podId||domainRow?.pod_id||null,inbox_id:random('inbox'),email:address,display_name:text(displayName,300)||null,client_id:text(clientId,300)||null,metadata:clone(safeObject(metadata)),domain_id:domainRow?.domain_id||null,paused:false,created_at:at(),updated_at:at()}; state.inboxes[row.inbox_id]=row; await emit(state,'inbox.created',row); return remember(state,idempotencyKey,clone(row)); }); },
    async listInboxes({ auth,limit=100,pageToken='',ascending=false }={}) { return mutate(async state=>{ await authContext(state,auth,'inbox_read'); let rows=Object.values(state.inboxes).map(clone); rows.sort((a,b)=>String(a.created_at).localeCompare(String(b.created_at))*(ascending?1:-1)); const out=page(rows,{limit,pageToken}); return{count:out.count,inboxes:out.items,limit:out.limit,next_page_token:out.next_page_token}; }); },
    async searchInboxes({ auth,q,limit=100,pageToken='' }={}) { return mutate(async state=>{ await authContext(state,auth,'inbox_read'); const query=text(q,256).toLowerCase(); if(query.length<2)throw new UberMailError('search-query-too-short',400); const terms=query.split(/\s+/).filter(Boolean); const rows=Object.values(state.inboxes).filter(r=>{const words=(r.email+' '+(r.display_name||'')).toLowerCase().split(/[^a-z0-9@._+-]+/).filter(Boolean);return terms.every(term=>r.email===term||words.some(w=>w.startsWith(term)));}).sort((a,b)=>(a.email===query?-1:b.email===query?1:0)); const out=page(rows,{limit,pageToken}); return{count:out.count,inboxes:out.items,limit:out.limit,next_page_token:out.next_page_token}; }); },
    ...
    async pollEvents({ auth,cursor=0,limit=100,eventTypes=[] }={}) { return mutate(async state=>{ await authContext(state,auth,'message_read');const start=Math.max(0,Number(cursor)||0);let rows=state.events.slice(start);const filter=new Set(array(eventTypes));if(filter.size)rows=rows.filter(r=>filter.has(r.type));rows=rows.slice(0,Math.max(1,Math.min(500,Number(limit)||100)));return{events:rows.map(eventPayload),cursor:start+rows.length,has_more:start+rows.length<state.events.length};}); },
    async metrics({ auth }={}) { return mutate(async state=>{ await authContext(state,auth,'metrics_read');return{...clone(state.metrics),inboxes:Object.keys(state.inboxes).length,domains:Object.keys(state.domains).length,pods:Object.keys(state.pods).length,messages:Object.keys(state.messages).length,threads:Object.keys(state.threads).length,drafts:Object.keys(state.drafts).length,webhooks:Object.keys(state.webhooks).length,api_keys:Object.values(state.apiKeys).filter(r=>!r.revoked).length,list_entries:Object.keys(state.listEntries).length};}); },
    async capabilityMatrix() { return {version:UBERMAIL_AGENT_API_VERSION,owned:true,externalSaasRequired:false,capabilities:{apiKeys:true,pods:true,domains:true,inboxes:true,inboxSearch:true,messages:true,messageSearch:true,replies:true,forwards:true,threads:true,threadSearch:true,drafts:true,scheduledDrafts:true,attachments:true,webhooks:true,allowBlockLists:true,inboundIngestion:true,eventPolling:true,metrics:true,durableRepository:'adapter',sendTransport:'pluggable-governed'}}; },
