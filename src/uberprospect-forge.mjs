...
export const UBERPROSPECT_VERSION='uberbond.uberprospect.v1';
const clean=(v,max=2000)=>String(v??'').trim().slice(0,max);
const sha=v=>crypto.createHash('sha256').update(String(v)).digest('hex');
const OFFERS=Object.freeze(['UBERREPLY_01','UBERREPLY_02','UBERREPLY_03','UBERREPLY_04']);
...
export function compileUberProspectPortfolio({records=[],target=2000,perOfferTarget=500,now=new Date()}={}){
  ...
  for(const row of records||[]){
    ...
    seen.add(key);
    const offerId=OFFERS[accepted.length%OFFERS.length];
  ...
  const laneCounts=Object.fromEntries(OFFERS.map(id=>[id,accepted.filter(x=>x.offerId===id).length]));
  const blockers=[];
  if(accepted.length<target) blockers.push('insufficient-governed-prospect-inventory');
  for(const id of OFFERS) if(laneCounts[id]<perOfferTarget) blockers.push(`${id.toLowerCase()}-inventory-shortfall`);
