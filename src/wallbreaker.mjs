...
export const WALLBREAKER_POLICY_VERSION = 'wallbreaker-1.1.0';
...
export function compileWallProblem(input = {}) {
  ...
  const requiredCapabilityAtomIds = list(input.requiredCapabilityAtomIds, 80, 160);
...
function normalizeCandidate(input = {}, index = 0) {
  ...
  const requiredCapabilityAtomIds = list(input.requiredCapabilityAtomIds, 60, 160);
  ...
  return {
    ...
    novelty,
    robustness,
    signature: digest({ family, mechanism, requiredCapabilities, requiredCapabilityAtomIds, assumptions }).slice(0, 32)
  };
}
...
export function scoreWallCandidate(input = {}, { problem = null, index = 0 } = {}) {
  ...
  const blocked = violatedHardConstraints.length > 0 || authorityViolation || spendViolation || founderMinuteViolation || riskViolation || mechanismMissing;
...
export function deriveCountermoves(failureInput = {}) {
  ...
  const capabilityQueries = failure.failureClass === 'CAPABILITY_GAP'
    ? [
        ...failure.missingCapabilities.map(capability => ({ capability, query: `best approved implementations for ${capability}`, atomId: null })),
        ...failure.missingCapabilityAtomIds.map(atomId => ({ capability: null, query: `approved capabilities covering atom ${atomId}`, atomId }))
      ]
    : [];
  const forbidden = [];
...
export function resolveWallCapabilities({ mission, requiredAtomIds = [], genome = null } = {}) {
  ...
  const retrieval = retrieveCapabilities({
    ...
    securityEvidenceByCapability: genome.securityEvidenceByCapability && typeof genome.securityEvidenceByCapability === 'object' ? genome.securityEvidenceByCapability : {},
...
export function planWallbreakerCycle(input = {}) {
  ...
  const failedSignatures = new Set(failures.filter(item => !item.safeToRetrySameMechanism).map(item => item.failedSignature).filter(Boolean));
  ...
  const computeTier = selectComputeTier({ eligibleCount: eligible.length, failureCount: failures.length, unknownCount: compiled.problem.unknowns.length, hardStopCount });
  ...
  const capabilityResolution = resolveWallCapabilities({ mission: compiled.problem.objective, requiredAtomIds: requiredCapabilityAtomIds, genome: input.genome ?? null });
  ...
  if (selected) status = capabilityBlocked ? 'CANDIDATE_SELECTED_CAPABILITY_GAP' : 'CANDIDATE_SELECTED';
