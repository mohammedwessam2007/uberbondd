...
function normalizeCompiledProblem(input = {}) {
  const looksCompiled = Boolean(input && typeof input === 'object' && !Array.isArray(input) && 'problemId' in input);
  if (!looksCompiled) return compileWallProblem(input);
  if (input.ok !== true || !input.problem || typeof input.problem !== 'object' || Array.isArray(input.problem)) {
    return invalid(['compiled-problem-envelope-invalid']);
  }
  const recompiled = compileWallProblem(input.problem);
  if (!recompiled.ok) return recompiled;
  const reasonCodes = [];
  if (input.policyVersion !== WALLBREAKER_POLICY_VERSION) reasonCodes.push('compiled-problem-policy-mismatch');
  if (input.problemId !== recompiled.problemId) reasonCodes.push('compiled-problem-id-mismatch');
  return reasonCodes.length ? invalid(reasonCodes, { problem: recompiled.problem }) : recompiled;
}
...
export function planWallbreakerCycle(input = {}) {
  const compiled = normalizeCompiledProblem(input.problem || {});
