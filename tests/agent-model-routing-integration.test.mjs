...
test('evidence routing selects only from workers already permitted by the activation gate', () => {
  const result = routePermittedWorkers({
    workers: [openai],
    benchmarks: [benchmark(openai), benchmark(anthropic, { quality: 1, reliability: 1 })],
    taskClass: 'coding', enabled: true, date: NOW, random: () => 0.99
  });
  assert.equal(result.ok, true);
  assert.equal(result.workers.length, 1);
  assert.equal(result.workers[0], openai);
  assert.equal(result.selected.workerId, 'openai-1');
});
...
test('higher evidence-backed score narrows an authorized rehearsal to one worker', () => {
  const result = routePermittedWorkers({
    ...
    benchmarks: [
      benchmark(openai, { quality: 0.7, reliability: 0.75, economicImpact: 0.6 }),
      benchmark(anthropic, { quality: 0.95, reliability: 0.95, economicImpact: 0.9 })
    ],
    taskClass: 'coding', enabled: true, date: NOW, random: () => 0.99
  });
  assert.equal(result.ok, true);
...
test('stale benchmark evidence cannot authorize routing', () => {
  ...
  const result = routePermittedWorkers({
    workers: [openai], benchmarks: [benchmark(openai, {}, stale)], taskClass: 'coding',
    enabled: true, maxBenchmarkAgeDays: 30, date: NOW, random: () => 0.99
  });
...
test('future-dated benchmark evidence cannot authorize routing', () => {
  ...
  const result = routePermittedWorkers({
    workers: [openai], benchmarks: [benchmark(openai, {}, future)], taskClass: 'coding',
    enabled: true, date: NOW, random: () => 0.99
  });
  assert.equal(result.ok, false);
  assert.ok(result.reasonCodes.includes('fresh-evidence-backed-route-required'));
});
...
test('low-confidence evidence cannot silently become a route', () => {
  const result = routePermittedWorkers({
    workers: [openai], benchmarks: [benchmark(openai, { evidenceConfidence: 0.2 })],
    taskClass: 'coding', enabled: true, minimumEvidenceConfidence: 0.5, date: NOW,
    random: () => 0.99
  });
  assert.equal(result.ok, false);
  assert.ok(result.reasonCodes.includes('fresh-evidence-backed-route-required'));
...
test('unbenchmarked exploration requires a separate explicit opt-in', () => {
  const blocked = routePermittedWorkers({
    workers: [openai], benchmarks: [], taskClass: 'coding', enabled: true,
    date: NOW, random: () => 0
  });
  assert.equal(blocked.ok, false);

  const allowed = routePermittedWorkers({
    workers: [openai], benchmarks: [], taskClass: 'coding', enabled: true,
    allowUnbenchmarkedExploration: true, explorationRate: 0.1,
    date: NOW, random: () => 0
  });
...
test('duplicate provider/model workers fail closed because routing identity is ambiguous', () => {
  const duplicate = { ...openai, workerId: 'openai-2' };
  const result = routePermittedWorkers({
    workers: [openai, duplicate], benchmarks: [benchmark(openai)],
    taskClass: 'coding', enabled: true, date: NOW
  });
  assert.equal(result.ok, false);
...
test('routing-enabled worker must declare a concrete provider and model', () => {
  const result = routePermittedWorkers({
    workers: [{ workerId: 'bad', provider: 'openai' }], benchmarks: [],
    taskClass: 'coding', enabled: true, date: NOW
  });
  assert.equal(result.ok, false);
  assert.ok(result.reasonCodes.includes('authorized-worker-must-declare-routable-provider-model'));
});
