...
import {
  validateEmployeeRoleRelayResultAdmission
} from '../src/ai-employee-relay-result-admission.mjs';

const ROLE_REF = 'engineering/capability-gap-analyst@1';
const ROLE_DIGEST = 'a'.repeat(64);

function roleTask(overrides = {}) {
  return {
    employeeRoleRef: ROLE_REF,
    employeeRoleDigest: ROLE_DIGEST,
    ...overrides
  };
}

function roleResult(overrides = {}) {
  return {
    employeeRoleRef: ROLE_REF,
    employeeRoleDigest: ROLE_DIGEST,
    ...overrides
...
function roleReceipt(overrides = {}) {
  return {
    employeeRoleRef: ROLE_REF,
    employeeRoleDigest: ROLE_DIGEST,
    ...overrides
...
test('incomplete task role identity fails closed', () => {
  const verdict = validateEmployeeRoleRelayResultAdmission({
    task: { employeeRoleRef: ROLE_REF },
    result: roleResult(),
    receipt: roleReceipt()
  });
  assert.equal(verdict.ok, false);
  assert.ok(verdict.reasonCodes.includes('relay-task-employee-role-identity-invalid'));
  assert.ok(verdict.reasonCodes.includes('task-employee-role-binding-incomplete'));
});

test('role-bound relay submission requires exact result identity', () => {
  const missing = validateEmployeeRoleRelayResultAdmission({
    task: roleTask(),
    result: {},
    receipt: roleReceipt()
  });
  ...
  assert.ok(missing.reasonCodes.includes('worker-result-employee-role-digest-required'));

  const substituted = validateEmployeeRoleRelayResultAdmission({
    task: roleTask(),
    result: roleResult({ employeeRoleRef: 'engineering/other@1' }),
    receipt: roleReceipt()
  });
...
test('role-bound relay submission requires terminal receipt identity', () => {
  const absent = validateEmployeeRoleRelayResultAdmission({
    task: roleTask(),
    result: roleResult(),
    receipt: null
  });
  ...
  const mismatched = validateEmployeeRoleRelayResultAdmission({
    task: roleTask(),
    result: roleResult(),
    receipt: roleReceipt({ employeeRoleDigest: 'b'.repeat(64) })
  });
...
test('nested receipt result cannot substitute employee identity', () => {
  const verdict = validateEmployeeRoleRelayResultAdmission({
    ...
    result: roleResult(),
    receipt: roleReceipt({
      result: roleResult({ employeeRoleDigest: 'c'.repeat(64) })
    })
  });
  assert.equal(verdict.ok, false);
