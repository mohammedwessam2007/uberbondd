...
test('layered store refuses poisoned archive before considering tail',()=>{const f=fixture();const bad=structuredClone(f.archive.manifest);bad.totalEntries=3;const out=compileLayeredCognitiveJournalStore({manifest:bad,segments:f.archive.segments,tailEntries:f.tailEntries});assert.equal(out.ok,false);assert.ok(out.reasonCodes.includes('verified-archive-segments-required'));});

test('empty archive tail uses genesis semantics',()=>{const all=entries(2);const out=verifyCognitiveJournalTail({archiveEntryCount:0,archiveTipDigest:null,tailEntries:all});assert.equal(out.ok,true);assert.equal(out.totalEntries,2);assert.equal(out.tipDigest,all.at(-1).entryDigest);});
