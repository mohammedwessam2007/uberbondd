...
test('status reports server-backed free-only model, relay and economic mission readiness without secrets',async()=>{
  const h=createHandler({env:{VERCEL_OIDC_TOKEN:'<redacted>',GITHUB_TOKEN:'<redacted>',GITHUB_REPOSITORY:'o/r',VERCEL_GIT_COMMIT_SHA:'a'.repeat(40),VERCEL_ENV:'preview',DATABASE_URL:'<redacted>'}});
...
test('ordinary chat still uses Vercel OIDC against the fixed free model',async()=>{
  let seen;
  const h=createHandler({env:{VERCEL_OIDC_TOKEN:'<redacted>',VERCEL_GIT_COMMIT_SHA:'b'.repeat(40)},root:'/definitely/missing',fetch:async(url,init)=>{seen={url,init,body:JSON.parse(init.body)};return {ok:true,status:200,text:async()=>JSON.stringify({choices:[{message:{content:'server answer'}}],usage:{total_tokens:12}})};}});
...
test('economic founder command is intercepted before conversational model and becomes mission state',async()=>{
  ...
  const h=createHandler({
    env:{VERCEL_OIDC_TOKEN:'<redacted>'},
...
test('economic bridge persists normalized mission without raw founder chat and dispatches durable jobs',async()=>{
  ...
  const result=await startEconomicOutcomeMission({
    env:{DATABASE_URL:'<redacted>',VERCEL_GIT_COMMIT_SHA:'d'.repeat(40)},
    message:privateText,
...
test('keep-working compiles a generic privacy-preserving relay task',async()=>{
  let call;
  const h=createHandler({env:{GITHUB_TOKEN:'<redacted>',GITHUB_REPOSITORY:'o/r',VERCEL_GIT_COMMIT_SHA:'c'.repeat(40)},fetch:async()=>{throw new Error('client should not execute in injected relay')},createRelayTask:async args=>{call=args;return {ok:true,status:'QUEUED',taskId:args.input.taskId,issueNumber:7,issueUrl:'https://github.test/7'}}});
...
test('cross-origin POST is refused before model, mission or relay work',async()=>{
  ...
  const h=createHandler({env:{VERCEL_OIDC_TOKEN:'<redacted>'},fetch:async()=>{throw new Error('must not call')},startOutcomeMission:async()=>{outcomeCalled=true;return {recognized:true}}});
