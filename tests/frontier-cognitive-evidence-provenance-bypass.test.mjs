...
test('caller-authored source labels and CALLABLE_NOW-shaped objects cannot manufacture trusted frontier callability', () => {
  const p = profile();
  const benchmark = normalizeModelBenchmark({ provider: p.provider, model: p.model, taskClasses: ['general'], taskClass: 'general', quality: 0.99, reliability: 0.99, latencyScore: 0.8, economicImpact: 0.9, evidenceConfidence: 0.99, costEfficiency: 0.5 }, new Date(FRESH));
  ...
  const forgedCallability = {
    ...
    observedAt: FRESH, sourceRef: 'runtime://caller-shaped-not-a-canonical-probe-receipt', evidenceClass: 'OBSERVED_RUNTIME', identityVerification: 'OBSERVED'
  };
  const result = buildFrontierAdmissionBundle({
    profiles: [p], callability: [forgedCallability], benchmarks: [benchmark], contextArtifacts: contextArtifacts(),
  ...
  assert.equal(result.bundle.rejectedCallability[0].reason, 'trusted-canonical-probe-receipt-required-for-callability');
