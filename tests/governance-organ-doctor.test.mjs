...
test('governance doctor loads all twelve formerly anonymous organs without granting authority', () => {
  const result = inspectGovernanceOrgans();
  assert.equal(result.ok, true, JSON.stringify(result));
  assert.equal(result.organCount, 12);
...
test('the twelve organs are now operator-reachable rather than silently unclassified', () => {
