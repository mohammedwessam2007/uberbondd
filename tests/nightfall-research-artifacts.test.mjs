...
test('current cloud contract records fresh connected-provider reachability with provenance', async () => {
  const contract = await readFile(new URL('../docs/CLOUD_ACTIVATION_CONTRACT.md', import.meta.url), 'utf8');
  assert.match(contract, /Observed provider state — 2026-09-09/);
  assert.match(contract, /prj_RWUPf14w1xIz9NK92AbNW5z7qDCg/);
  assert.match(contract, /prj_ZMfDCuUva2kdMv6HnqGvIE5vihTz/);
  assert.match(contract, /dpl_2PhVeSieAtWWADw4pdxavwd1ULZR/);
  assert.match(contract, /08be0cd10a6e8e35d5c5dfa608ec0f7ad5583c2e/);
  assert.match(contract, /historical connector-scope contradiction/i);
});
...
test('current cloud contract does not promote project reachability into production or scheduler liveness', async () => {
  ...
  assert.match(contract, /Current zero-I\/O cron registration proven \| \*\*NO\*\*/);
  assert.match(contract, /Current zero-I\/O cron delivery proven \| \*\*NO\*\*/);
  assert.match(contract, /Production deployments authorized or performed by this repair lane \| \*\*NONE\*\*/);
  assert.doesNotMatch(contract, /Current main exact production SHA proven \| \*\*YES\*\*/);
});
