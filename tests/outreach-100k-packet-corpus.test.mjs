...
const packet=(i,box='m1')=>({
  ...
  launchInput:{recipient:{email:`p${i}@example.com`,verified:true},campaign:{id:'c1'},legal:{eligible:true},suppression:{checked:true,suppressed:false,unsubscribeRequested:false}}
});
...
test('exact scheduled corpus compiles digest inventory and provider counts',async()=>{
  ...
  const r=await inspectOutreach100kPacketCorpus({filePath:f,mailboxes:boxes,campaignId:'c1',expectedCount:3});
...
test('duplicate recipient refuses corpus',async()=>{
  const a=packet(1),b=packet(2);b.message.to=a.message.to;b.launchInput.recipient.email=a.message.to;
  const f=await file([a,b]);
  const r=await inspectOutreach100kPacketCorpus({filePath:f,mailboxes:boxes,campaignId:'c1',expectedCount:2});
