...
test('retryable billing event clears claim and schedules a bounded retry',async()=>{
  ...
  const out=await finishBillingEvent(pool,{providerEventKey:'evt',status:'RETRYABLE',errorCode:'provider-timeout',retryAfterMs:60000});
  assert.equal(out.status,'RETRYABLE');
  assert.ok(out.nextAttemptAt);
  assert.match(calls[0][0],/claimed_at=CASE WHEN \$2='RETRYABLE' THEN NULL/);
