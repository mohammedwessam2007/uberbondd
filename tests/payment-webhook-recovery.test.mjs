...
test('same provider event with changed money is quarantined and never resumes', async () => {
  const { store, dir } = await tempStore();
  const config = cfg(dir);
  const { engine, lead, prospect } = await seedLead(store, config);
  const first = payload({ eventName: 'order_created', eventId: 'evt_mismatch', leadId: lead.id, prospectId: prospect.id, amountCents: 4900 });
  const changed = payload({ eventName: 'order_created', eventId: 'evt_mismatch', leadId: lead.id, prospectId: prospect.id, amountCents: 9900 });

  const restore = crashNextPaymentClassification(store);
  await assert.rejects(() => deliver(engine, first), /injected-crash-after-order-before-classification/);
  restore();

  const result = await deliver(engine, changed);
  assert.equal(result.failClosed, true);
  assert.equal(result.reason, 'provider-event-witness-mismatch');
  assert.equal((await store.list('revenueEvents')).length, 0);
  const [order] = await store.list('orders');
  assert.equal(order.completionStatus, 'quarantined');
});

test('a contradictory ledger witness cannot grant paid access during recovery', async () => {
  const { store, dir } = await tempStore();
  const config = cfg(dir);
  const { engine, pipeline, lead, prospect } = await seedLead(store, config);
  const event = payload({ eventName: 'order_created', eventId: 'evt_ledger_conflict', leadId: lead.id, prospectId: prospect.id });
  ...
  await store.add('revenueEvents', {
    id: 'rev-conflicting-witness', providerEventId: 'order_created:evt_ledger_conflict',
    leadId: lead.id, prospectId: prospect.id, product: 'full', kind: 'sale',
