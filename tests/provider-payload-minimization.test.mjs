...
test('signed provider webhook persists the bounded normalized witness, not the raw customer payload', async () => {
  ...
  const payload = {
    ...
    data: {
      ...
      attributes: {
        ...
        session_token: 'provider-session-secret-that-must-not-be-durable',
  ...
  for (const forbidden of [
    ...
    'provider-session-secret-that-must-not-be-durable'
  ]) {
  ...
  assert.equal(persisted.providerEventId, 'evt_privacy_1');
