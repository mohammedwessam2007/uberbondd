...
test('JSON omission cannot hide an undefined post-signature mutation',()=>{const x=input(),a=attest(x);x.controlPlaneReceipt.forged=undefined;assert.equal(verifyRuntimeEvidenceAttestation({attestation:a,input:x,publicKeyPem:publicPem}),false);});
test('JSON null coercion cannot hide non-finite post-signature mutation',()=>{const x=input();x.restartRecoveryReceipt.observed.optionalMetric=null;const a=attest(x);x.restartRecoveryReceipt.observed.optionalMetric=NaN;assert.equal(verifyRuntimeEvidenceAttestation({attestation:a,input:x,publicKeyPem:publicPem}),false);});
test('executable or exotic evidence values fail closed rather than entering the signed data language',()=>{for(const mutate of [x=>{x.controlPlaneReceipt.fn=()=>1;},x=>{x.controlPlaneReceipt.when=new Date();},x=>{Object.defineProperty(x.controlPlaneReceipt,'computed',{enumerable:true,get(){return 'x';}});},x=>{x.controlPlaneReceipt.big=1n;}]){const x=input();mutate(x);const payload=runtimeEvidenceAttestationPayload(x);assert.equal(payload.acceptanceEvidenceDigests.controlPlane,null);assert.equal(verifyRuntimeEvidenceAttestation({attestation:{version:RUNTIME_EVIDENCE_ATTESTATION_VERSION},input:x,publicKeyPem:publicPem}),false);}});
...
test('cyclic evidence fails closed instead of crashing canonicalization',()=>{const x=input();x.restartRecoveryReceipt.loop=x.restartRecoveryReceipt;const payload=runtimeEvidenceAttestationPayload(x);assert.equal(payload.acceptanceEvidenceDigests.restartRecovery,null);assert.equal(verifyRuntimeEvidenceAttestation({attestation:{version:RUNTIME_EVIDENCE_ATTESTATION_VERSION},input:x,publicKeyPem:publicPem}),false);});
test('over-deep evidence fails closed instead of exhausting canonicalization',()=>{const x=input();let cursor=x.controlPlaneReceipt;for(let i=0;i<70;i++){cursor.next={};cursor=cursor.next;}const payload=runtimeEvidenceAttestationPayload(x);assert.equal(payload.acceptanceEvidenceDigests.controlPlane,null);assert.equal(verifyRuntimeEvidenceAttestation({attestation:{version:RUNTIME_EVIDENCE_ATTESTATION_VERSION},input:x,publicKeyPem:publicPem}),false);});
