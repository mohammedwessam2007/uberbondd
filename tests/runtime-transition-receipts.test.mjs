...
test('observer cannot attest wrong transition kind even with recomputed digest',()=>{const r=observed(durable(),'DURABLE_WORKLOAD');r.observerReceipt.transitionKind='CUTOVER_ROLLBACK';r.observerReceipt.receiptDigest=h(runtimeTransitionObserverReceiptPreimage(r.observerReceipt));assert.equal(verifyRuntimeTransitionObserverReceipt(r.observerReceipt,r.assertion),false);assert.equal(verifyRuntimeTransitionReceiptIntegrity(r,'DURABLE_WORKLOAD'),false);});
test('observer from different source revision cannot attest assertion',()=>{const r=observed(durable(),'DURABLE_WORKLOAD');r.observerReceipt.producerSourceCommit='c'.repeat(40);r.observerReceipt.receiptDigest=h(runtimeTransitionObserverReceiptPreimage(r.observerReceipt));assert.equal(verifyRuntimeTransitionObserverReceipt(r.observerReceipt,r.assertion),false);});
test('receipt observation time cannot drift from observer',()=>{const r=observed(durable(),'DURABLE_WORKLOAD');r.observedAt='2026-09-09T03:12:00Z';r.receiptDigest=h(runtimeTransitionObservedReceiptPreimage(r,'DURABLE_WORKLOAD'));assert.equal(verifyRuntimeTransitionReceiptIntegrity(r,'DURABLE_WORKLOAD'),false);});
