...
test('Sandwich Method doctor exercises exact-head evidence-bound folding, blueprint evolution and local task compilation without external claims', () => {
  ...
  assert.equal(report.blueprintEvolutionReachable, true);
  assert.equal(report.blueprintSyntheticRevision, 'r2-synthetic-doctor');
  ...
  assert.match(report.truthBoundary, /EVIDENCE_BINDING_BLUEPRINT_EVOLUTION_AND_LOCAL_TASK_COMPILATION_ONLY/);
