...
} from '../.github/workflows/runtime/self-maintainer-proposal-contract.mjs';
...
function task(overrides = {}) {
  return {
    ...
    constraints: [`exact-base-revision:${BASE}`, 'one-bounded-change-set', 'local-preparation-only', 'business-effect-authority:none'],
...
function proposal(overrides = {}) {
  return {
    ...
    baseRevision: BASE,
    changes: [{ operation: 'UPDATE', path: 'src/example-safe-module.mjs', beforeSha256: BEFORE, content: 'export const oldValue = 2;\n', rationale: 'Make the smallest causal source repair.' }],
...
test('proposal profile has one explicit name and a closed raw schema', () => {
  ...
  assert.deepEqual([...SELF_MAINTAINER_RAW_PROPOSAL_SCHEMA.required].sort(), ['baseRevision', 'changes', 'cognitivePrioritiesConsidered', 'decision', 'evidenceRefs', 'summary', 'verification'].sort());
});
...
test('exact-base mismatch is rejected before compilation', () => {
  const out = compileSelfMaintainerProposalWorkerResult({ task: task(), proposal: proposal({ baseRevision: 'b'.repeat(40) }) });
  assert.equal(out.ok, false);
  assert.ok(out.reasonCodes.includes('proposal-base-revision-mismatch'));
});

test('protected runtime compiler is itself uneditable through the canonical self-maintainer contract', () => {
  const out = compileSelfMaintainerProposalWorkerResult({ task: task(), proposal: proposal({ changes: [{ ...proposal().changes[0], path: '.github/workflows/runtime/self-maintainer-proposal-contract.mjs' }] }) });
  assert.equal(out.ok, false);
...
test('protected and sovereignty paths remain impossible through the canonical compiler', () => {
  for (const path of ['package.json', 'scripts/run-tests.mjs', 'src/agent-code-change-contract.mjs', 'scripts/uberbond-self-maintainer-tick.mjs']) {
    const out = compileSelfMaintainerProposalWorkerResult({ task: task(), proposal: proposal({ changes: [{ ...proposal().changes[0], path }] }) });
...
test('missing before hash, secret-bearing content and verification weakening fail closed', () => {
  const noHash = compileSelfMaintainerProposalWorkerResult({ task: task(), proposal: proposal({ changes: [{ ...proposal().changes[0], beforeSha256: '' }] }) });
  assert.equal(noHash.ok, false);
  assert.ok(noHash.reasonCodes.some(code => code.includes('before-hash-required')));
  const secret = compileSelfMaintainerProposalWorkerResult({ task: task(), proposal: proposal({ changes: [{ ...proposal().changes[0], content: `const OPENAI_API_KEY = '${'sk-proj-' + 'z'.repeat(48)}';\n` }] }) });
...
test('STOP path carries no edits and no fake verification claims', () => {
  const out = compileSelfMaintainerProposalWorkerResult({ task: task(), proposal: proposal({ decision: 'STOP', changes: [], summary: 'No safe worthwhile repair found.' }) });
  assert.equal(out.ok, true);
...
test('STOP cannot smuggle edits', () => {
  const out = compileSelfMaintainerProposalWorkerResult({ task: task(), proposal: proposal({ decision: 'STOP' }) });
  assert.equal(out.ok, false);
