...
let eventCounter = 0;
// `date` is the trusted clock and `at` is the caller's claim about when
// something happened; the machine refuses a claim that runs ahead of the clock.
// A test simulating "a day later" therefore has to move the clock, not just the
// label -- which is what elapsed time means. Moving only the label is the
// fast-forward attack, and it is now covered by its own test.
function apply(state, type, extra = {}, at = '2026-08-23T01:00:00Z', clock = at) {
  eventCounter += 1;
  return applyFulfillmentEvent({ state, event: { eventId: `evt_${eventCounter}`, type, at, ...extra }, date: clock });
}
