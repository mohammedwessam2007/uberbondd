...
test('resident host selects once, does not duplicate while pending, then advances from durable evidence without a new prompt',async()=>{
  ...
  const runtimeRunner=async()=>{
    runtimeCalls+=1;
    if(runtimeCalls===1){await writeJson(activeCyclePath,cycle());return{ok:true,initiativeAdmitted:true,status:'NORTH_STAR_INITIATIVE_SELECTED',sourceCommit:BASE,executionReceipt:null,paths:{}};}
    ...
    return{ok:true,initiativeAdmitted:true,status:'NORTH_STAR_INITIATIVE_ADVANCED_WITHOUT_NEW_PROMPT',sourceCommit:BASE,executionReceipt:{receiptId:'execution-proof'},paths:{}};
  };
