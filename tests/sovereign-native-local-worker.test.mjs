...
test('local model configuration only accepts owner-host loopback and explicitly enables the worker',()=>{const cfg=readFileSync(new URL('../ops/sovereign/configure-local-model.sh',import.meta.url),'utf8');assert.match(cfg,/127\\\.0\\\.0\\\.1\|localhost\|\\\[::1\\\]/);assert.match(cfg,/UBERBOND_ISOLATED_WORKER_ENABLED=true/);assert.match(cfg,/UBERBOND_FOUNDER_DIALOGUE_ENABLED=true/);assert.match(cfg,/OPEN_MODEL_AGENT_ENABLED=true/);});
