...
test('couriers signed bundle atomically without granting signing or deployment authority',async()=>{
  const f=await fixture();const out=await runSovereignReleaseCourier({env:{UBERBOND_RELEASE_COURIER_SOURCE:f.source,UBERBOND_RELEASE_COURIER_RUNTIME_INBOX:f.inbox}});
...
test('refuses unsafe marker names',async()=>{const f=await fixture();await writeFile(path.join(f.source,'NEXT_RELEASE'),'../escape\n');const out=await runSovereignReleaseCourier({env:{UBERBOND_RELEASE_COURIER_SOURCE:f.source,UBERBOND_RELEASE_COURIER_RUNTIME_INBOX:f.inbox}});assert.equal(out.ok,false);assert.ok(out.reasonCodes.includes('safe-release-name-required'));});

test('refuses symlink anywhere in signed bundle',async()=>{const f=await fixture();await symlink('/etc/passwd',path.join(f.rel,'evil'));const out=await runSovereignReleaseCourier({env:{UBERBOND_RELEASE_COURIER_SOURCE:f.source,UBERBOND_RELEASE_COURIER_RUNTIME_INBOX:f.inbox}});assert.equal(out.ok,false);assert.ok(out.reasonCodes.includes('release-tree-symlink-refused'));});
