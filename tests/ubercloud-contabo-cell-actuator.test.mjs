...
import { compileContaboMailCellPlan, executeContaboMailCellAcquisition, createContaboApiClient } from '../src/ubercloud-contabo-cell-actuator.mjs';
...
test('plan is zero-effect and binds current quote, exact source and smallest selected product',()=>{
  ...
  assert.equal(result.status,'CONTABO_MAIL_CELL_PLAN_READY');
  assert.equal(result.plan.request.productId,'V153');
  assert.equal(result.plan.request.period,1);
  assert.equal(result.plan.sourceCommit,SHA);
...
test('missing or insufficient founder spend authorization prevents all provider calls',async()=>{
  ...
  const client={async createInstance(){calls++;return{ok:true,instanceId:'1'};}};
...
test('authorized acquisition records one bounded purchase and leaves PTR/runtime/DNS truth pending',async()=>{
  const p=plan();let calls=0;
  ...
  assert.equal(result.status,'CONTABO_MAIL_CELL_ACQUIRED__PHYSICAL_RECONCILIATION_REQUIRED');
  assert.equal(result.receipt.instanceId,'instance-123');
  assert.equal(result.receipt.ptrStatus,'PENDING_OBSERVATION_OR_UPDATE');
  assert.equal(result.externalEffectLedger.providerCalls,1);
  assert.equal(result.externalEffectLedger.purchases,1);
  assert.equal(result.externalEffectLedger.deployments,1);
  assert.equal(result.externalEffectLedger.spendCents,1000);
  assert.equal(result.receipt.truthBoundary.includes('does not prove PTR'),true);
});
...
test('provider exception after authorization claim becomes uncertain rather than retry-safe failure',async()=>{
  ...
  assert.equal(result.ok,false);assert.equal(result.status,'CONTABO_MAIL_CELL_ACQUISITION_UNCERTAIN');
  assert.equal(result.spendAuthority,'CONSUMED');
  assert.equal(result.externalEffectLedger.providerCalls,null);
  assert.equal(result.externalEffectLedger.purchases,null);
});

test('HTTP client authenticates in memory and never places credentials in request URLs',async()=>{
  const calls=[];
  const fetchFn=async(url,options={})=>{
    ...
    if(String(url).endsWith('/v1/compute/instances'))return{ok:true,status:201,json:async()=>({instanceId:123,publicIpv4:'203.0.113.25'})};
  ...
  assert.equal(calls[1].options.headers.authorization,'Bearer token-abc');
