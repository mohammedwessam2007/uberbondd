...
const cell=(overrides={})=>({
  cellId:'owned-a',resourceType:'EXECUTION',provider:'owned-a',sourceRef:'receipt:cell:a',verifiedAt:NOW,
...
test('UberCloud refuses to call one provider sovereign without a distinct fallback',()=>{
  ...
  assert.ok(result.blocked.some(row=>row.reasonCodes.includes('distinct-provider-fallback-required')));
});
...
test('UberCloud refuses provider-native networking as the root for founder-private state',()=>{
  ...
  const state=(id,provider,networkMode)=>cell({cellId:id,provider,resourceType:'DATABASE',sourceRef:`receipt:${id}`,capabilityTags:['postgres'],allowedDataClasses:['FOUNDER_PRIVATE'],networkMode,portableExport:true,openFormat:true,stateFormat:'pg_dump',restoreProcedureRef:'ops:restore-postgres'});
  const result=compileUberCloudPlan({serviceId:'private-memory',requirements:[req],cells:[state('db-a','vendor-a','PROVIDER_NATIVE'),state('db-b','vendor-b','PROVIDER_NATIVE')],meshReceipt:mesh,maxTotalCostCents:0});
  assert.equal(result.ok,false);
  assert.ok(result.blocked.some(row=>row.reasonCodes.includes('distinct-provider-fallback-required'))||result.blocked.some(row=>row.reasonCodes.includes('primary-state-not-portable')));
});
...
test('UberCloud requires portable state and provider-independent UberMesh evidence',()=>{
  ...
  const locked=cell({cellId:'db-a',provider:'vendor-a',resourceType:'DATABASE',capabilityTags:['postgres'],allowedDataClasses:['INTERNAL_NON_SECRET'],portableExport:false,openFormat:false,stateFormat:null,restoreProcedureRef:null});
  const portable=cell({cellId:'db-b',provider:'vendor-b',resourceType:'DATABASE',capabilityTags:['postgres'],allowedDataClasses:['INTERNAL_NON_SECRET'],portableExport:true,openFormat:true,stateFormat:'pg_dump',restoreProcedureRef:'ops:restore'});
...
test('UberCloud compiles and verifies an evacuation path without granting provider authority',()=>{
  const result=compileUberCloudPlan({
    serviceId:'continuum',requirements:[execReq],meshReceipt:mesh,maxTotalCostCents:0,
    cells:[cell({cellId:'owned-a',provider:'owned-a'}),cell({cellId:'open-b',provider:'open-b',sourceRef:'receipt:cell:b',ownershipClass:'OPEN_SELF_HOSTED',reliability:.95})]
  });
  ...
  const evacuation=compileUberCloudEvacuation({planResult:result,failedProviders:[result.plan.placements[0].primary.provider]});
