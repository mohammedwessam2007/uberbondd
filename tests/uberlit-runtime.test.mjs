...
function fixture(){
  ...
  const server=label=>`import http from 'node:http';\nconst port=Number(process.env.PORT);\nconst server=http.createServer((req,res)=>{if(req.url==='/api/health'){res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify({ok:true,label:${JSON.stringify(label)},releaseId:process.env.UBERLIT_RELEASE_ID,sourceCommit:process.env.UBERLIT_SOURCE_COMMIT}));return;}res.writeHead(200);res.end(${JSON.stringify(label)});});server.listen(port,'127.0.0.1');process.on('SIGTERM',()=>server.close(()=>process.exit(0)));`;
...
test('staged source is self-contained exact git source and tracked mutation refuses',async()=>{const f=fixture();try{const m=manifest(f.v1);const staged=stageUberLitRelease({rootDir:f.root,repoDir:f.repo,manifest:m});assert.equal(staged.ok,true);const source=path.join(f.root,'releases',m.releaseId,'source');assert.equal(git(source,['rev-parse','HEAD']),f.v1);assert.equal(git(source,['remote']), '');fs.appendFileSync(path.join(source,'server.mjs'),'\n// tamper');const refused=verifyStagedUberLitRelease({rootDir:f.root,releaseId:m.releaseId});assert.equal(refused.ok,false);assert.ok(refused.reasonCodes.includes('tracked-source-dirty'));}finally{await f.cleanup();}});

test('process cannot start before a verified build receipt exists',async()=>{const f=fixture();try{const m=manifest(f.v1);stageUberLitRelease({rootDir:f.root,repoDir:f.repo,manifest:m});assert.throws(()=>startUberLitRelease({rootDir:f.root,releaseId:m.releaseId,port:await freePort()}),/verified-build-required/);}finally{await f.cleanup();}});
...
test('health-gated promotion runs exact source and rollback restores previous release',async()=>{const f=fixture();const port=await freePort();try{const m1=manifest(f.v1),m2=manifest(f.v2);for(const m of [m1,m2]){stageUberLitRelease({rootDir:f.root,repoDir:f.repo,manifest:m});buildUberLitRelease({rootDir:f.root,releaseId:m.releaseId,port});}
    const first=await promoteUberLitRelease({rootDir:f.root,releaseId:m1.releaseId,port,healthTimeoutMs:5000});assert.equal(first.ok,true);assert.equal(first.pointer.sourceCommit,f.v1);let response=await fetch(`http://127.0.0.1:${port}/`);assert.equal(await response.text(),'v1');
    const second=await promoteUberLitRelease({rootDir:f.root,releaseId:m2.releaseId,port,healthTimeoutMs:5000});assert.equal(second.pointer.sourceCommit,f.v2);response=await fetch(`http://127.0.0.1:${port}/`);assert.equal(await response.text(),'v2');
    const rolled=await rollbackUberLitRelease({rootDir:f.root,healthTimeoutMs:5000});assert.equal(rolled.pointer.sourceCommit,f.v1);response=await fetch(`http://127.0.0.1:${port}/`);assert.equal(await response.text(),'v1');
  }finally{await f.cleanup();}});

test('failed health admission never advances active pointer',async()=>{const f=fixture();const port=await freePort();try{const good=manifest(f.v1);stageUberLitRelease({rootDir:f.root,repoDir:f.repo,manifest:good});buildUberLitRelease({rootDir:f.root,releaseId:good.releaseId,port});await promoteUberLitRelease({rootDir:f.root,releaseId:good.releaseId,port,healthTimeoutMs:5000});const before=readUberLitPointer({rootDir:f.root});
    const bad=compileUberLitRelease({sourceCommit:f.v2,buildArgv:[],startArgv:[process.execPath,'server.mjs'],healthPath:'/never-healthy',processRole:'web'});stageUberLitRelease({rootDir:f.root,repoDir:f.repo,manifest:bad});buildUberLitRelease({rootDir:f.root,releaseId:bad.releaseId,port});await assert.rejects(()=>promoteUberLitRelease({rootDir:f.root,releaseId:bad.releaseId,port,healthTimeoutMs:700}),/health-admission-failed/);const after=readUberLitPointer({rootDir:f.root});assert.equal(after.releaseId,before.releaseId);assert.equal(after.sourceCommit,f.v1);
  }finally{await f.cleanup();}});
