...
const endpoint = (recipientId, endpointId, channel = 'WEB_CONTACT') => ({ recipientId, endpointId, channel, observedAt: '2026-09-15T23:30:00.000Z', evidenceRef: `receipt:${endpointId}`, publicOrAuthorized: true, platformTermsCompatible: true, suppressed: false });
...
test('deduplicates recipients so route multiplicity cannot inflate reach', () => {
  const opportunities = [{ opportunityId: 'o1', recipientId: 'r1' }, { opportunityId: 'o2', recipientId: 'r1' }];
...
test('refuses unauthorized endpoints', () => {
  const r = compileObjectiveSubstitution({ opportunities: [{ opportunityId: 'o1', recipientId: 'r1' }], endpoints: [{ ...endpoint('r1', 'e1'), publicOrAuthorized: false }], target: 1, now: NOW });
  assert.equal(r.state, 'NO_LEGITIMATE_REACH');
  assert.equal(r.uniqueReachableRecipients, 0);
});

test('unready SMTP certificate never becomes send authority', () => {
  const r = compileObjectiveSubstitution({ opportunities: [{ opportunityId: 'o1', recipientId: 'r1' }], endpoints: [endpoint('r1', 'e1', 'ATTENTION_API')], target: 1, now: NOW, smtpCertificate: { state: 'WAIT_EXTERNAL_EVIDENCE', oneButton100kPressAvailable: false } });
