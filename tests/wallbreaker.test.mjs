...
test('forged or malformed compiled problem envelopes cannot bypass compilation gates', () => {
  const compiled = compileWallProblem(baseProblem);
  const mutated = {
    ...compiled,
    problem: { ...compiled.problem, maxSpendCents: 99_999_999 }
  };
  const forgedPlan = planWallbreakerCycle({ problem: mutated, candidates: [candidate({ costCents: 9000 })] });
  assert.equal(forgedPlan.ok, false);
  assert.ok(forgedPlan.reasonCodes.includes('compiled-problem-id-mismatch'));

  const malformedPlan = planWallbreakerCycle({ problem: { problemId: 'wall_forged' }, candidates: [candidate()] });
  assert.equal(malformedPlan.ok, false);
  assert.ok(malformedPlan.reasonCodes.includes('compiled-problem-envelope-invalid'));
  assert.deepEqual(malformedPlan.externalEffectLedger, ZERO_EXTERNAL_EFFECTS);
});
