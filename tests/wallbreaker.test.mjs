...
test('a failed mechanism with uncertain external outcome is not blindly retried and a different family wins', () => {
  ...
  const alternate = candidate({
    id: 'alternate',
    family: 'dependency-redesign',
    mechanism: 'Remove the failed provider dependency entirely.',
    expectedContributionCents: 8500,
    evidenceRefs: ['architecture-proof']
  });
  ...
  const plan = planWallbreakerCycle({
    ...
    failures: [{
      ...
      candidateId: 'failed',
      failedSignature: failedScore.candidate.signature,
      outcomeUncertain: true,
      evidenceRefs: ['provider-outage']
    }]
  });
...
test('falsified assumptions prune dependent candidates and reopen search', () => {
  const plan = planWallbreakerCycle({
    ...
    failures: [{
      assumptionFalsified: true,
      invalidatedAssumptions: ['provider-a-available'],
      evidenceRefs: ['provider-a-down']
    }]
  });
...
test('unclear walls escalate compute while retaining zero external effects', () => {
  const plan = planWallbreakerCycle({
    problem: {
      ...baseProblem,
      unknowns: Array.from({ length: 9 }, (_, i) => `unknown-${i}`)
    },
    candidates: []
  });
