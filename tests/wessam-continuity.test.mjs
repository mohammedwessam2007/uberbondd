...
test('recovery capsule requires fresh owner identity and refuses revoked device/provider targets', () => {
  ...
  const revokedProvider = compileRecoveryCapsule({
    ...
    ownerAuthorization: { ownerId: source.ownerId, authorizationDigest: 'x'.repeat(64) },
    targetBoundaryId: 'new-private-fabric',
    targetProviderId: 'revoked-cloud'
  });
...
test('recovery capsule contains references only and transfers no authority', () => {
  const source = state();
  const result = compileRecoveryCapsule({
    state: source,
    ownerAuthorization: { ownerId: source.ownerId, authorizationDigest: 'z'.repeat(64) },
    targetBoundaryId: 'replacement-private-fabric',
    targetDeviceId: 'new-ipad'
  });
  assert.equal(result.ok, true, JSON.stringify(result));
...
test('external task compiler sends only requested minimum-sufficient allowed memory', () => {
  const result = compileExternalTaskPacket({
    taskId: 'research-1',
    purpose: 'research current model pricing',
    requestedFields: ['public-fact', 'internal-note'],
...
test('WESSAM_INNERMOST plaintext cannot be externalized even if requested', () => {
  const result = compileExternalTaskPacket({
    taskId: 'forbidden',
    purpose: 'attempt innermost export',
    requestedFields: ['neural-raw'],
    allowedPrivacyClasses: ['WESSAM_INNERMOST'],
    memoryItems: [{ key: 'neural-raw', privacyClass: 'WESSAM_INNERMOST', value: 'raw future BCI data', provenanceRef: 'sensor:1' }]
  });
...
test('secret-like values and secret-bearing keys are excluded from external task packets', () => {
  const result = compileExternalTaskPacket({
    taskId: 'secret-test',
    purpose: 'must redact secrets',
    requestedFields: ['bad1', 'bad2', 'safe'],
    memoryItems: [
...
test('restored state rejects stale epoch, memory drift, identity drift and authority inflation', () => {
  ...
  restored.recoveryEpoch = 2;
  restored.memoryRootDigest = 'changed';
  restored.ownerId = 'other';
  restored.selfAuthorityEscalationAllowed = true;
  const result = verifyRestoredWessamState({
    sourceState: source,
    restoredState: restored,
    ownerAuthorization: { ownerId: source.ownerId }
  });
  assert.equal(result.ok, false);
  for (const code of ['owner-changed-during-restore', 'memory-root-drift', 'stale-recovery-epoch', 'authority-inflation-during-restore']) {
