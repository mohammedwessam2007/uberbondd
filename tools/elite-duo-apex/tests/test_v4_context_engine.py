...
# Runtime hooks/statusline live at the repo root's .claude/, not under a
# tools/elite-duo-apex/claude/ mirror; scripts/ remains vendored library source.
...
class V4Tests(unittest.TestCase):
 ...
 def test_evaporation(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   p=subprocess.run([sys.executable,str(CLAUDE_DIR/'hooks/tool_output_evaporator.py')],input=json.dumps(payload),text=True,capture_output=True,cwd=CLAUDE_DIR/'hooks',env=env)
 ...
 def test_postcompact(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   env=dict(os.environ); env['CLAUDE_PROJECT_DIR']=d
   p=subprocess.run([sys.executable,str(CLAUDE_DIR/'hooks/postcompact_v4.py')],input=json.dumps(payload),text=True,capture_output=True,cwd=CLAUDE_DIR/'hooks',env=env)
 ...
 def test_benchmark_rejects_quality_loss(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   # Explicit --output keeps the generated report inside the temp dir instead
   # of the script's cwd-relative default landing in the tracked tree.
   p=subprocess.run([sys.executable,str(PKG_ROOT/'scripts/benchmark_fraction_engine.py'),str(b),str(c),'--output',str(Path(d)/'FRACTION_ENGINE_BENCHMARK.json')],text=True,capture_output=True)
